    ...
    : taskType === 'meme'
      ...
      : taskType === 'plan'
        ...
        : taskType === 'likes'
          ...
          : taskType === 'adopt'
          ? `【本轮创作任务：纳取】围绕 User 指定的气泡，把值得保留的创作意图改写成 AI 可读的开放式提示词，而不是替正文写摘要或导演角色反应。必须输出恰好3个气泡且按顺序：1) builtin:writer 以“【导演版】”开头，偏现实条件、事件入口、外部机会与被规划人物自身可采取的行动空间；2) builtin:guide 以“【灵感版】”开头，事件创意可以更大胆、更意外，但仍不得预定任何角色的心理/反应/结果；3) builtin:writer 以“【二人合璧】”开头，融合自然性约束与事件创意，主动删除前两版里任何角色结论，不是简单拼接。三版都必须能允许正文中的角色拒绝、无视、误解、采取完全不同反应，甚至让事件最终没有重要影响。${taskNotes ? `\n纳取原文：${taskNotes}` : ''}`
