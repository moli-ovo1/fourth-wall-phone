...
async function buildBatchGroupRequest({
  ...
  const studioPersonaOverlay = member => {
    ...
    return `【编辑室专属人格覆盖｜只在“先磕点瓜子再说”生效】
...
悬置：${String(thread?.unresolved || '暂无').trim()}`;
  ...
  const studioTaskGuidance = !studio ? '' : taskType === 'cast'
    ? `【本轮创作任务：Ta出场太少啦】目标不是“下一场硬塞 Ta 出来一次”，而是恢复指定配角作为独立人物在世界中的自然活动与持续存在感。只从 Ta 自己已经存在的人设、职业/职责、关系、利益、承诺、日程、事务与当前环境出发，为 Ta 提供近期可以自然联系、出现、场外活动或推进自身事务的开放机会；允许这些活动最终没有影响主角或主线。不得为了刺激主角而安排 Ta 精准撞上关键场面，不得为了增加戏份强闯，也不得让 Ta 获得不应知道的信息。尤其禁止规划“Ta 的出现将导致其他角色怎样想、怎样选、怎样回应”；规划配角，不规划配角造成的结果。${studioThreadSupervision}${taskNotes ? `\n用户补充：${taskNotes}` : ''}`
    : taskType === 'meme'
      ? `【本轮创作任务：帮我想梗】
正文AI真是太无聊啦！当用户点击这个入口时，你们帮她想想梗吧！
有什么可能自然发生的小事、偶遇、麻烦、便利、插曲、意外获得、环境变化或他人的独立活动？可以平淡、荒诞、温柔、扫兴、麻烦、幸运、尴尬，甚至没有主线意义；生活允许只是发生。近期已反复使用的同类机关应主动降权。提供事件入口，不预设各角色的心理，不预设后续发展。${studioThreadSupervision}${taskNotes ? `\n用户补充：${taskNotes}` : ''}`
        : taskType === 'likes'
