...
import { listPublicWebPosts, createPublicWebPost, addPublicWebPosts, getPublicWebPost, addPublicWebComment, togglePublicWebLike, deletePublicWebPost, getPublicWebSettings, updatePublicWebSettings, togglePublicWebFavorite, togglePublicWebPinned, replacePublicWebSectionPosts, appendPublicWebSectionPosts, trimPublicWebSectionPosts, forceDeletePublicWebPost, deletePublicWebComment, deleteZhihuAnswerComment, deleteZhihuAnswer, addZhihuAnswerComments, addZhihuAnswer, mergeZhihuRefresh, listPublicWebFavorites, listCustomCommunities, ensureCustomCommunityPresets, deleteCustomCommunities, saveCustomCommunity, deleteCustomCommunity, getCommunityUserProfile, updateCommunityUserProfile, listWeiboFollows, saveWeiboFollow, deleteWeiboFollow, isWeiboFollowed, incrementWeiboRepost, setWeiboFollowHot, listWeiboPrivateMessages, addWeiboPrivateMessage, deleteWeiboPrivateMessage, clearWeiboPrivateMessages, getWeiboMessageReadState, markWeiboMessageRead, listWeiboFanGroups, saveWeiboFanGroup, addWeiboFanGroupMessage, listWeiboMessagePeers } from '../storage/public-web-store.js';
...
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择小号时使用的小号网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
  ...
  panel.querySelector('[data-action="public-web-refresh"]')?.addEventListener('click', async event => {
    ...
    try {
      ...
      if (currentPublicWebTab === 'recommend') {
        ...
        if(currentPublicWebTab==='weibo'){appendPublicWebSectionPosts(getScopeKey?.(),'weibo',items);trimPublicWebSectionPosts(getScopeKey?.(),'weibo',20);}else replacePublicWebSectionPosts(getScopeKey?.(),currentPublicWebTab,items);
      }
  ...
  const openWeiboComposeDialog=()=>{
    ...
    overlay.querySelector('[data-action="weibo-publish"]').onclick=()=>{const content=String(body.value||'').trim();const images=media.filter(x=>x.type==='image').map(x=>x.text).slice(0,9);const videos=media.filter(x=>x.type==='video').map(x=>x.text).slice(0,1);if(!content&&!images.length&&!videos.length){windowRef.alert?.('写点内容或添加图片/视频后再发送。');return;}const userName=getTavernUserContext()?.name||'User';const tags=[...content.matchAll(/#[^#\s]{1,40}#/g)].map(m=>m[0]).slice(0,12);createPublicWebPost(getScopeKey?.(),{section:'weibo',type:'weibo',author:{type:'user',id:'user',name:userName},title:content.slice(0,40),content,tags,extra:{weiboLane:'实时',images,videos,userOwned:true,reposts:0,hotScore:0}});close();renderPublicWeb();}; body.focus();
  };
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event => {
    ...
    const weiboRefresh=event.target?.closest?.('[data-action="public-web-refresh"]'); if(weiboRefresh&&currentPublicWebTab==='weibo'){if(publicWebGenerating.has('weibo'))return;publicWebGenerating.add('weibo');renderPublicWeb();try{const items=await generatePublicWebRefresh({scopeKey:getScopeKey?.(),section:'weibo',weiboMode:currentWeiboView});appendPublicWebSectionPosts(getScopeKey?.(),'weibo',items);openedPublicWebPostId='';renderPublicWeb();}catch(error){console.error('[moli小手机] weibo refresh failed:',error);windowRef.alert?.(`刷新失败：${error?.message||error}`);}finally{publicWebGenerating.delete('weibo');renderPublicWeb();}return;}
