...
export function createPhonePanel({
  ...
  const renderPublicWeb = () => {
    ...
    if (tianyaChrome) {
      ...
      feed.innerHTML=`<div class="moli-xhs-home"><div class="moli-xhs-home-head"><span></span><span></span></div><div class="moli-xhs-waterfall">${cards||'<div class="moli-recommend-empty">点右上角 ↻ 刷新笔记。</div>'}</div><button class="moli-xhs-compose-fab" data-action="public-web-compose" aria-label="发布笔记">＋</button></div>`;
    } else {
  ...
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event => {
    ...
    const zhihuInvite=event.target?.closest?.('[data-action="zhihu-invite-answer"]'); if(zhihuInvite){windowRef.alert?.('“邀请回答 → 微信角色自主决定 ANSWER / MESSAGE / BOTH / SKIP”将在下一步接入；本轮先完成知乎原生问答结构。');return;}
