...
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择匿名时使用的匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
  ...
  const renderPublicWebDetail = post => {
    ...
    if (post.section === 'weibo') {
      const name=communityAuthorDisplay(post.author);const aid=String(post.author?.id||post.author?.name||'');const isFollowed=isWeiboFollowed(getScopeKey?.(),aid)||isWeiboFollowed(getScopeKey?.(),name);const imgs=(Array.isArray(post.extra?.images)?post.extra.images:[post.extra?.imagePrompt].filter(Boolean)).slice(0,9);const media=imgs.length?`<div class="moli-weibo-media moli-weibo-media-${Math.min(imgs.length,9)}">${imgs.map((x,i)=>`<div><small>图片${i+1}</small><span>${escapeHtml(x)}</span></div>`).join('')}</div>`:'';const commentRows=comments.map(c=>`<div class="moli-weibo-comment" data-community-comment="1" data-post-id="${escapeHtml(post.id)}" data-comment-id="${escapeHtml(c.id)}"><b>${escapeHtml(communityAuthorDisplay(c.author))}</b><p>${escapeHtml(c.content||'')}</p><button data-action="public-web-comment" data-post-id="${escapeHtml(post.id)}">回复</button></div>`).join('');
  ...
  const renderPublicWeb = () => {
    ...
    if (tianyaChrome) {
      ...
      const mediaFor=post=>{const imgs=(Array.isArray(post.extra?.images)?post.extra.images:[post.extra?.imagePrompt].filter(Boolean)).slice(0,9);return imgs.length?`<div class="moli-weibo-media moli-weibo-media-${Math.min(imgs.length,9)}">${imgs.map((x,i)=>`<div><small>图片${i+1}</small><span>${escapeHtml(x)}</span></div>`).join('')}</div>`:'';};
