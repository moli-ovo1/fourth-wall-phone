...
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择小号时使用的小号网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
  ...
  const openWeiboSearchDialog=()=>{
    ...
    overlay.querySelector('[data-action="weibo-search-submit"]').onclick=async()=>{const q=String(overlay.querySelector('[data-weibo-search-input]')?.value||'').trim();if(!q)return;const btn=overlay.querySelector('[data-action="weibo-search-submit"]');btn.disabled=true;btn.textContent='搜索中…';try{const items=await generatePublicWebRefresh({scopeKey:getScopeKey?.(),section:'weibo',weiboQuery:q,weiboMode:'search'});appendPublicWebSectionPosts(getScopeKey?.(),'weibo',items);close();renderPublicWeb();const hotBtn=panel.querySelector('[data-weibo-view="hot"]');hotBtn?.click();}catch(error){windowRef.alert?.(`搜索失败：${error?.message||error}`);btn.disabled=false;btn.textContent='搜索';}};
