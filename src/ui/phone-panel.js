...
export function createPhonePanel({
  ...
  const renderPublicWeb = () => {
    ...
    if (tianyaChrome) {
      ...
      feed.innerHTML=`<div class="moli-custom-home"><h2>${escapeHtml(userName)}小窝</h2><p>写下你想在这个世界里看到的内容；启用后会和天涯、小红书、知乎一起参与“社区推荐”刷新。</p>
        <section class="moli-custom-library">
          ...
          <div class="moli-custom-library-body" ${customListOpen?'':'hidden'}>
            <div class="moli-custom-manage-row"><label><input type="checkbox" data-custom-select-all> 全选</label><button type="button" data-action="custom-delete-selected" disabled>删除</button></div>
            <button type="button" class="moli-custom-new" data-action="custom-new">＋ 新增</button>
        ...
        <section class="moli-custom-posts"><h3>自创内容</h3>${postRows||'<div class="moli-web-muted">社区推荐生成的自创内容会自动收进这里。</div>'}</section>
      </div>`;
  ...
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', event=>{
    ...
    const all=event.target.closest('[data-recommend-all]'); if(all){panel.querySelectorAll('[data-recommend-source]').forEach(x=>x.checked=all.checked);panel.querySelectorAll('[data-recommend-custom]').forEach(x=>x.checked=all.checked);const sources=[...panel.querySelectorAll('[data-recommend-source]:checked')].map(x=>x.dataset.recommendSource);const customIds=[...panel.querySelectorAll('[data-recommend-custom]:checked')].map(x=>x.dataset.recommendCustom);const count=Number(panel.querySelector('[data-recommend-count]')?.value||0)||0;updatePublicWebSettings(scope,{recommendSources:sources,recommendCustomIds:customIds,recommendCount:count});return;}
