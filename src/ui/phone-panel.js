...
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择匿名时使用的匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
  ...
  const openWeiboComposeDialog=()=>{
    ...
    const body=overlay.querySelector('[data-weibo-compose-text]'); const hint=overlay.querySelector('[data-weibo-compose-at-hint]'); const media=[];
    ...
    body.addEventListener('input',()=>{const before=body.value.slice(0,body.selectionStart);hint.hidden=!/@$/.test(before);});hint.addEventListener('click',()=>{hint.hidden=true;body.focus();});
    ...
    overlay.querySelector('[data-action="weibo-insert-at"]').onclick=()=>{body.setRangeText('@',body.selectionStart,body.selectionEnd,'end');hint.hidden=false;body.focus();};
