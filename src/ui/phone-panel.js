...
${continuity?`【你自己的手机经历/认知】\n${continuity}\n`:''}${item.type==='invite'?`User 明确邀请你${isAnswer?'回答这个问题':'参与跟帖'}`:`User 在评论中明确 @了你：${item.content||''}`}。这是 User 的明确互动，你必须${isAnswer?'回答':'回应'}，不允许 SKIP。${forcedMode==='real'?'User @的是你的本名，本次必须使用本名公开回应。':forcedMode==='surface'&&forcedAlias?`User @的是你在本帖已经使用的公开账号“${forcedAlias}”，本次必须继续使用“${forcedAlias}”回应，不得切换为本名，也不得暴露该账号背后的真实身份。`:'你只判断使用 REPLY_REAL（实名）还是 REPLY_ANONYMOUS（匿名）；匿名表示使用匿名网名公开回应。'}${proactiveEnabled?'公开回应完成后，再独立判断是否需要私聊 User；SEND 时给 2~5 条真实手机气泡，SKIP 时可给简短原因。':'主动私聊权限关闭：PRIVATE 必须 SKIP。'}
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择匿名时使用的匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
      try{
        const result=await generatePrivateReply({scopeKey,conversationKey,automationInstruction:instruction,allowNoPendingUser:true}); const raw=String(result?.text||'');
