...
export function createPhonePanel({
  ...
  const consumeUserCommentPending = (postId,answerId=null) => { const rows=readCommunityPending().filter(x=>x.type==='user-comment'&&String(x.postId||'')===String(postId||'')&&(answerId===null||String(x.answerId||'')===String(answerId||''))); markWorldEventsKnownByObjectTargets(getScopeKey?.(),rows.map(x=>x.commentId)); consumePending(rows.map(x=>x.id)); };
  const settleQueuedCommunityTargets = async post => {
    ...
    for(const item of targeted){
      ...
      try{
        ...
        const resultEvent=recordWorldEvent(scopeKey,{source:`community.${post.section||'unknown'}`,actorId:target.id,action:isAnswer?'PUBLIC_ANSWER':'PUBLIC_REPLY',targetContactIds:[target.id],objectId:String(post.id||''),content:`你在${sourceLabel(post)}公开${isAnswer?'回答':'回应'}了 User 的明确互动。`,metadata:{postId:String(post.id||''),pendingId:String(item.id||''),identityMode:identity},awareness:'known',dedupeKey:`community-role:${item.id}`}); if(identity==='ANONYMOUS')rememberAnonymousIdentity(scopeKey,{surface:`community.${post.section||'unknown'}`,alias,realContactId:target.id,knownBy:[target.id],evidenceEventId:resultEvent?.id});
        markWorldEventsConsumedByObjectTargets(scopeKey,[item.id],'community-settlement');
        done.push(item.id);
  ...
  const renderPublicWeb = () => {
    ...
    if (tianyaChrome) {
      ...
      feed.innerHTML = `<div class="moli-recommend-home">
        ...
        <header class="moli-recommend-today"><button type="button" class="moli-recommend-refresh${publicWebGenerating.has('recommend')?' is-spinning':''}" data-action="public-web-refresh-recommend" aria-label="刷新社区推荐">↻</button><span>今天的社区发生了什么……</span></header>
