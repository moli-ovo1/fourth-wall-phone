  ...
  panel.querySelector('[data-action="public-web-refresh"]')?.addEventListener('click', async event => {
    ...
    try {
      ...
      if (currentPublicWebTab === 'recommend') {
        ...
        if(currentPublicWebTab==='weibo'){appendPublicWebSectionPosts(getScopeKey?.(),'weibo',items);trimWeiboLanePosts(getScopeKey?.(),15);}else replacePublicWebSectionPosts(getScopeKey?.(),currentPublicWebTab,items);
      }
