...
${item.type==='invite'?`User 明确邀请你${isAnswer?'回答这个问题':'参与跟帖'}`:`User 在评论中明确 @了你：${item.content||''}`}。这是 User 的明确互动，你必须${isAnswer?'回答':'回应'}，不允许 SKIP。你只判断使用 REPLY_REAL（实名）还是 REPLY_ANONYMOUS（匿名）；匿名表示使用匿名网名公开回应。${proactiveEnabled?'公开回应完成后，再独立判断是否需要私聊 User；SEND 时给 2~5 条真实手机气泡，SKIP 时可给简短原因。':'主动私聊权限关闭：PRIVATE 必须 SKIP。'}
