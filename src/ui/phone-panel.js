...
export function createPhonePanel({
  ...
  const consumeUserCommentPending = (postId,answerId=null) => { const ids=readCommunityPending().filter(x=>x.type==='user-comment'&&String(x.postId||'')===String(postId||'')&&(answerId===null||String(x.answerId||'')===String(answerId||''))).map(x=>x.id);consumePending(ids); };
  const settleQueuedCommunityTargets = async post => {
    ...
    for(const item of targeted){
      ...
      const scopeKey=getScopeKey?.(); const conversationKey=privateConversationKeyFor(scopeKey,target.id); const isAnswer=item.kind==='answer';
      const instruction=`【moli社区刷新结算｜${isAnswer?'邀请回答':'明确互动'}】\n平台：${sourceLabel(post)}\n标题/问题：${post.title||'无标题'}\n正文/补充：${post.content||'无'}\n当前讨论：\n${communityDiscussionContext(post)}\n\n${item.type==='invite'?`User 明确邀请你${isAnswer?'回答这个问题':'参与跟帖'}`:`User 在评论中明确 @了你：${item.content||''}`}。这是明确互动，刷新结算后你必须公开回应，不允许 SKIP。你可选择 REAL 本名或 ANONYMOUS 匿名；匿名时请取网名。严格追加 <community_identity>REAL|ANONYMOUS</community_identity><community_alias>匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}。`;
      try{
        ...
        const identity=(raw.match(/<community_identity>\s*(REAL|ANONYMOUS)\s*<\/community_identity>/i)?.[1]||'REAL').toUpperCase(); const alias=String(raw.match(/<community_alias>([\s\S]*?)<\/community_alias>/i)?.[1]||'').trim()||'匿名用户';
        ...
        if(isAnswer){const text=String(raw.match(/<community_answer>([\s\S]*?)<\/community_answer>/i)?.[1]||'').trim();if(text)addZhihuAnswer(scopeKey,post.id,{author,content:text});}
        else {const text=String(raw.match(/<community_reply>([\s\S]*?)<\/community_reply>/i)?.[1]||'').trim();if(text){if(post.section==='zhihu'&&item.answerId)addZhihuAnswerComments(scopeKey,post.id,item.answerId,[{author,content:text,replyToCommentId:String(item.replyToCommentId||'')}]);else addPublicWebComment(scopeKey,post.id,{author,content:text,replyToCommentId:String(item.replyToCommentId||'')});}}
        done.push(item.id);
      }catch(error){console.error('[moli小手机] queued community interaction failed:',error);throw error;}
    }
    consumePending(done); renderPublicWeb();
  };
  ...
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event => {
    ...
    const communityInvite=event.target?.closest?.('[data-action="public-web-invite"]'); if(communityInvite){
      ...
      queueCommunityPending({type:'invite',kind:'comment',postId:String(post.id),targetId:String(choice.item.id)});
      recordWorldEvent(getScopeKey?.(),{source:`community.${post.section}`,actorId:'user',action:'INVITE_COMMENT',targetContactIds:[choice.item.id],objectId:String(post.id||''),content:`User 邀请你参与社区帖子：${post.title||'无标题'}`,metadata:{postId:String(post.id||''),pendingRefresh:true},awareness:'known'});
    ...
    const zhihuInvite=event.target?.closest?.('[data-action="zhihu-invite-answer"]'); if(zhihuInvite){
      ...
      queueCommunityPending({type:'invite',kind:'answer',postId:String(post.id),targetId:String(choice.item.id)});
      recordWorldEvent(getScopeKey?.(),{source:'community.zhihu',actorId:'user',action:'INVITE_ANSWER',targetContactIds:[choice.item.id],objectId:String(post.id||''),content:`User 邀请你回答知乎问题：${post.title}`,metadata:{postId:String(post.id||''),pendingRefresh:true},awareness:'known'});
      toast(`已邀请${displayName(choice.item)}，点击回答区刷新后统一结算`); return;
    }
    const zhihuAnswersRefresh=event.target?.closest?.('[data-action="zhihu-answers-refresh"]'); if(zhihuAnswersRefresh){const post=getPublicWebPost(getScopeKey?.(),zhihuAnswersRefresh.dataset.postId);if(!post)return;try{await settleQueuedCommunityTargets(post);renderPublicWeb();}catch(error){windowRef.alert?.(`刷新回答失败：${error?.message||error}`);}return;}
    ...
    const share=event.target?.closest?.('[data-action="public-web-share"]'); if(share){
      ...
      recordWorldEvent(scopeKey,{source:`community.${post.section||'custom'}`,actorId:'user',action:'FORWARD',targetContactIds:choice.kind==='group'?(target.memberIds||[]):[target.id],objectId:String(post.id||''),content:`User 转发给你一条${platform}内容：${post.title||'无标题'}`,metadata:{postId:String(post.id||''),customCommunityId:String(post.extra?.customCommunityId||'')},awareness:'known'});
  ...
  communityComposer.querySelector('[data-community-send]')?.addEventListener('click',()=>{const state=communityComposerState;if(!state)return;const post=getPublicWebPost(getScopeKey?.(),state.postId);if(!post)return;const content=String(communityComposer.querySelector('[data-community-content]')?.value||'').trim();if(!content)return;const mode=state.identityMode||'real';let author;if(mode==='anonymous'){const alias=String(communityComposer.querySelector('[data-community-alias]')?.value||'').trim()||'匿名用户';setAnonymousAlias(state.postId,alias);author={type:'user',id:'user',name:alias,anonymous:true,knownIdentityId:'user',identityKnownBy:['user']};}else if(mode==='owner'){author={type:'user',id:'user',name:'楼主',uiName:'楼主',anonymous:false,knownIdentityId:'user'};}else author={type:'user',id:'user',name:getTavernUserContext()?.name||'User',anonymous:false};const entry={id:`userc_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,author,content,replyToCommentId:state.replyToCommentId};let saved;if(post.section==='zhihu'&&state.composeMode==='zhihu-answer'){saved={id:`zua_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,author,content,comments:[],upvotes:0};addZhihuAnswer(getScopeKey?.(),post.id,saved);}else if(post.section==='zhihu'&&state.zhihuAnswerId){saved={...entry};addZhihuAnswerComments(getScopeKey?.(),post.id,state.zhihuAnswerId,[saved]);}else saved=addPublicWebComment(getScopeKey?.(),post.id,entry);const mentions=Array.isArray(state.mentionTargets)?state.mentionTargets:[];const answerId=state.zhihuAnswerId;if(saved&&state.composeMode!=='zhihu-answer')queueCommunityPending({type:'user-comment',postId:String(post.id),answerId:String(answerId||''),commentId:String(saved.id||''),content:String(saved.content||'')});for(const mention of mentions)queueCommunityPending({type:'mention',kind:'comment',postId:String(post.id),answerId:String(answerId||''),replyToCommentId:String(saved?.id||''),commentId:String(saved?.id||''),content:String(saved?.content||''),targetId:String(mention.id)});closeCommunityComposer();renderPublicWeb();});
