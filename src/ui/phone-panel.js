...
export function createPhonePanel({
  ...
  const applyUnifiedCommunitySettlement = async (post, result, pending, { answerId = '' } = {}) => {
    const scopeKey=getScopeKey?.(); const done=[]; const notices=[];
    for(const action of (result?.targetActions||[])){
      const item=(pending||[]).find(x=>String(x.id)===String(action.pendingId)); const target=getContacts().find(x=>String(x.id)===String(action.targetId)); if(!target)continue;
      ...
      recordCommunityPostSnapshotAwareness(scopeKey,post,[target.id],item?'explicit-participation':'natural-participation',Date.now());
      const participantIds=new Set(); const addParticipant=a=>{const id=String(a?.knownIdentityId||a?.id||'');if(id&&id!=='user'&&getContacts().some(c=>String(c.id)===id))participantIds.add(id);};
      addParticipant(post.author); for(const c of (post.comments||[]))addParticipant(c.author); if(post.section==='zhihu')for(const a of (post.extra?.answers||[])){addParticipant(a.author);for(const c of (a.comments||[]))addParticipant(c.author);}
      const coParticipants=[...participantIds].filter(id=>id!==String(target.id));
      if(coParticipants.length)recordWorldEvent(scopeKey,{source:`community.${post.section||'unknown'}`,actorId:target.id,action:'CHARACTER_PUBLIC_INTERACTION',targetContactIds:coParticipants,objectId:String(post.id||''),content:`${displayName(target)}刚刚在你也参与的${sourceLabel(post)}讨论中继续公开发言。`,metadata:{postId:String(post.id||''),causedByEventId:String(resultEvent?.id||''),unresolved:true},awareness:'known',dedupeKey:`community-participant-awareness:${post.id}:${target.id}:${Date.now()}`});
      if(privateDid)recordWorldEvent(scopeKey,{source:`community.${post.section||'unknown'}`,actorId:target.id,action:'PRIVATE_MESSAGE_SENT',targetContactIds:[target.id],objectId:String(post.id||''),content:`你因这次${sourceLabel(post)}互动主动私聊了 User。`,metadata:{postId:String(post.id||''),pendingId:String(item?.id||''),causedByEventId:String(resultEvent?.id||'')},awareness:'known',dedupeKey:`community-private-experience:${item?.id||post.id}:${target.id}:${Date.now()}`});
      if(identity==='ANONYMOUS')rememberAnonymousIdentity(scopeKey,{surface:`community.${post.section||'unknown'}`,alias,realContactId:target.id,knownBy:[target.id],evidenceEventId:resultEvent?.id});
      if(item){markWorldEventsKnownByObjectTargets(scopeKey,[item.id]); markWorldEventsConsumedByObjectTargets(scopeKey,[item.id],'community-settlement'); done.push(item.id);} notices.push(`${displayName(target)}${identity==='ANONYMOUS'?'已匿名':'已实名'}回应`);
    }
    for(const c of (result?.comments||[])){
      if(post.section==='zhihu'&&answerId) addZhihuAnswerComments(scopeKey,post.id,answerId,[c]); else addPublicWebComment(scopeKey,post.id,c);
    }
    if(post.section==='zhihu'){
      for(const a of (result?.answers||[])) addZhihuAnswer(scopeKey,post.id,a);
      for(const batch of (result?.commentAdditions||[])) addZhihuAnswerComments(scopeKey,post.id,batch.answerId,batch.comments);
    }
    consumePending(done); consumeUserCommentPending(post.id, answerId || null); if(notices.length)windowRef.alert?.(notices.join('\n')); return result;
  };
  const runUnifiedCommunityRefresh = async (post, { mode='thread', answerId='' } = {}) => {
    const pending=pendingForPost(post?.id); const result=await generateCommunitySettlement({scopeKey:getScopeKey?.(),post,pending,mode,answerId}); await applyUnifiedCommunitySettlement(post,result,pending,{answerId}); return result;
  };
...
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择匿名时使用的匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 1~3 个 <msg>私聊内容</msg>。`;
  ...
  const deleteCommunityCommentFromTarget = target => { const row=target?.closest?.('[data-community-comment]'); if(!row)return false; const postId=String(row.dataset.postId||''),commentId=String(row.dataset.commentId||''); if(!postId||!commentId)return false; if(windowRef.confirm?.('删除这条评论？')){deletePublicWebComment(getScopeKey?.(),postId,commentId);renderPublicWeb();} return true; };
  panel.querySelector('[data-public-web-feed]')?.addEventListener('contextmenu', event => { if(deleteCommunityCommentFromTarget(event.target)){event.preventDefault();event.stopPropagation();} });
  let communityLongPressTimer=0,communityLongPressTarget=null;
  panel.querySelector('[data-public-web-feed]')?.addEventListener('touchstart',event=>{const row=event.target?.closest?.('[data-community-comment]');if(!row)return;communityLongPressTarget=row;clearTimeout(communityLongPressTimer);communityLongPressTimer=setTimeout(()=>{deleteCommunityCommentFromTarget(communityLongPressTarget);communityLongPressTarget=null;},650);},{passive:true});
  for(const type of ['touchend','touchcancel','touchmove'])panel.querySelector('[data-public-web-feed]')?.addEventListener(type,()=>{clearTimeout(communityLongPressTimer);communityLongPressTarget=null;},{passive:true});
  ...
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event => {
    ...
    const zhihuAnswersRefresh=event.target?.closest?.('[data-action="zhihu-answers-refresh"]'); if(zhihuAnswersRefresh){const postId=String(zhihuAnswersRefresh.dataset.postId||'');const busyKey=`zhihu-detail:${postId}`;if(publicWebGenerating.has(busyKey))return;publicWebGenerating.add(busyKey);renderPublicWeb();try{const post=getPublicWebPost(getScopeKey?.(),postId);if(!post)return;await runUnifiedCommunityRefresh(post,{mode:'zhihu-detail'});renderPublicWeb();}catch(error){windowRef.alert?.(`刷新知乎失败：${error?.message||error}`);}finally{publicWebGenerating.delete(busyKey);renderPublicWeb();}return;}
    ...
    const replyRefresh=event.target?.closest?.('[data-action="tianya-replies-refresh"]'); if(replyRefresh){const postId=String(replyRefresh.dataset.postId||'');const busyKey=`tianya-comments:${postId}`;if(publicWebGenerating.has(busyKey))return;publicWebGenerating.add(busyKey);renderPublicWeb();try{const post=getPublicWebPost(getScopeKey?.(),postId);await runUnifiedCommunityRefresh(post,{mode:'replies'});renderPublicWeb();}catch(error){console.error('[moli小手机] tianya replies refresh failed:',error);windowRef.alert?.(`新增回复失败：${error?.message||error}`);}finally{publicWebGenerating.delete(busyKey);renderPublicWeb();}return;}
