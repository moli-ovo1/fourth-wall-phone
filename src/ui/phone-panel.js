...
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择匿名时使用的匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
  ...
  const openWeiboMessagesDialog=()=>{
    const scopeKey=getScopeKey?.(); const overlay=document.createElement('div'); overlay.className='moli-weibo-overlay';
    const renderList=()=>{const follows=listWeiboFollows(scopeKey);overlay.innerHTML=`<div class="moli-weibo-messages-dialog"><div class="moli-weibo-dialog-head"><b>私信 / 关注</b><button data-weibo-dialog-close>×</button></div><div class="moli-weibo-message-tabs"><b>关注人</b><small>🔥 持续互动账号会保存微博互动与私信记忆</small></div><div class="moli-weibo-follow-list">${follows.map(x=>`<div class="moli-weibo-follow-row"><button class="moli-weibo-follow-open" data-follow-open="${escapeHtml(x.id)}"><b>${escapeHtml(x.name)}</b>${x.hot?'<em>🔥</em>':''}<small>${escapeHtml(x.profile||'暂无账号画像')}</small></button><button data-follow-hot="${escapeHtml(x.id)}" aria-label="持续互动">${x.hot?'🔥':'○'}</button></div>`).join('')||'<p class="moli-web-muted">还没有关注账号。</p>'}</div></div>`;overlay.querySelector('[data-weibo-dialog-close]').onclick=()=>overlay.remove();overlay.querySelectorAll('[data-follow-hot]').forEach(b=>b.onclick=()=>{const row=follows.find(x=>String(x.id)===String(b.dataset.followHot));if(row){setWeiboFollowHot(scopeKey,row.id,!row.hot);renderList();}});overlay.querySelectorAll('[data-follow-open]').forEach(b=>b.onclick=()=>renderChat(String(b.dataset.followOpen)));};
    const renderChat=id=>{const account=listWeiboFollows(scopeKey).find(x=>String(x.id)===String(id));if(!account)return renderList();const msgs=listWeiboPrivateMessages(scopeKey,id);overlay.innerHTML=`<div class="moli-weibo-messages-dialog is-chat"><div class="moli-weibo-dialog-head"><button data-dm-back>‹</button><b>@${escapeHtml(account.name)} ${account.hot?'🔥':''}</b><button data-dm-edit>编辑</button></div><div class="moli-weibo-dm-profile">${escapeHtml(account.profile||'暂无账号画像')}</div><div class="moli-weibo-dm-list">${msgs.map(m=>`<div class="${m.role==='user'?'is-user':'is-account'}">${escapeHtml(m.content)}</div>`).join('')||'<p class="moli-web-muted">还没有私信。</p>'}</div><div class="moli-weibo-dm-compose"><textarea data-dm-input rows="2" placeholder="发私信…"></textarea><button data-dm-send>发送</button></div></div>`;overlay.querySelector('[data-dm-back]').onclick=renderList;overlay.querySelector('[data-dm-edit]').onclick=()=>{const next=windowRef.prompt?.(`编辑 @${account.name} 的账号设定`,account.profile||'');if(next!==null){saveWeiboFollow(scopeKey,{...account,profile:String(next||'').trim()});renderChat(id);}};overlay.querySelector('[data-dm-send]').onclick=async()=>{const ta=overlay.querySelector('[data-dm-input]');const text=String(ta.value||'').trim();if(!text)return;addWeiboPrivateMessage(scopeKey,id,{role:'user',content:text});renderChat(id);const btn=overlay.querySelector('[data-dm-send]');if(btn){btn.disabled=true;btn.textContent='…';}try{const latest=listWeiboFollows(scopeKey).find(x=>String(x.id)===id)||account;const reply=await generateWeiboPrivateReply({scopeKey,account:latest,messages:listWeiboPrivateMessages(scopeKey,id)});if(reply)addWeiboPrivateMessage(scopeKey,id,{role:'account',content:reply});}catch(error){windowRef.alert?.(`私信回复失败：${error?.message||error}`);}renderChat(id);};};
    panel.appendChild(overlay);overlay.addEventListener('click',e=>{if(e.target===overlay)overlay.remove();});renderList();
  };
  ...
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event => {
    ...
    const follow=event.target?.closest?.('[data-action="weibo-follow"]'); if(follow){const scopeKey=getScopeKey?.();const id=String(follow.dataset.accountId||follow.dataset.accountName||'').trim(),name=String(follow.dataset.accountName||id).trim();const row=listWeiboFollows(scopeKey).find(x=>String(x.id)===id||String(x.name)===name);if(row){deleteWeiboFollow(scopeKey,row.id);renderPublicWeb();return;}saveWeiboFollow(scopeKey,{id,name,profile:'',source:'observed'});renderPublicWeb();const related=listPublicWebPosts(scopeKey,{section:'weibo'}).filter(p=>String(p.author?.id||p.author?.name||'')===id||String(p.author?.name||'')===name).slice(0,5);const evidence=related.map(p=>String(p.content||p.title||'')).filter(Boolean).join('；').slice(0,500);void summarizeWeiboAccountProfile({scopeKey,name,evidence}).then(profile=>{if(isWeiboFollowed(scopeKey,id)||isWeiboFollowed(scopeKey,name))saveWeiboFollow(scopeKey,{id,name,profile,source:'observed'});}).catch(()=>{});return;}
