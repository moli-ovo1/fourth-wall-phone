...
export function createPhonePanel({
  ...
  const renderRecommendFilter=(settings)=>{
    ...
    return `<div class="moli-recommend-filter-panel" ${recommendFilterOpen?'':'hidden'}>
      <div class="moli-recommend-filter-row moli-recommend-filter-top"><span class="moli-recommend-default-note">未设置时按默认全部来源</span><label>帖子条数 <input class="moli-recommend-count" type="number" min="1" max="20" data-recommend-count value="${customized?escapeHtml(String(settings.recommendCount||'')):''}" placeholder="默认"></label></div>
  ...
  const renderPublicWeb = () => {
    ...
    if (tianyaChrome) {
      ...
      feed.innerHTML=`<div class="moli-custom-home">
        ...
        <section class="moli-custom-posts"><h3>帖子</h3>${customRows(customOrdinary)||'<div class="moli-web-muted">社区推荐生成的自创内容会自动收进这里。</div>'}</section>
        <section class="moli-custom-posts moli-custom-pinned"><h3>常驻</h3>${customRows(customPinned)||'<div class="moli-web-muted">点进帖子后用 ☺ 把它留在这里。</div>'}</section>
      </div>`;
