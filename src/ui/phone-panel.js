...
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择匿名时使用的匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
  ...
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event=>{
    const scope=getScopeKey?.();
    const userHomeToggle=event.target.closest('[data-action="custom-user-home-toggle"]');if(userHomeToggle){const body=panel.querySelector('.moli-custom-user-home-body');if(body)body.hidden=!body.hidden;return;} const idManager=event.target.closest('[data-action="community-id-manager"]');if(idManager){const scopeKey=getScopeKey?.();const overlay=document.createElement('div');overlay.className='moli-weibo-overlay moli-community-id-overlay';const renderIds=()=>{const profile=getCommunityUserProfile(scopeKey);overlay.innerHTML=`<div class="moli-community-id-dialog"><div class="moli-weibo-dialog-head"><b>全社区ID管理</b><button data-id-close>×</button></div><div class="moli-community-id-add"><select data-id-type><option>大号</option><option selected>小号</option></select><input data-id-name placeholder="输入社区ID"><button data-id-add>保存</button></div><div class="moli-community-id-list">${profile.communityIds.map(x=>`<button data-id-delete="${escapeHtml(x.id)}">（${escapeHtml(x.type)}）${escapeHtml(x.name)}</button>`).join('')||'<p class="moli-web-muted">还没有保存社区ID。</p>'}</div><p class="moli-web-muted">长按名字可删除。</p></div>`;overlay.querySelector('[data-id-close]').onclick=()=>overlay.remove();overlay.querySelector('[data-id-add]').onclick=()=>{const name=String(overlay.querySelector('[data-id-name]')?.value||'').trim();if(!name)return;const type=String(overlay.querySelector('[data-id-type]')?.value||'小号');const ids=getCommunityUserProfile(scopeKey).communityIds.filter(x=>x.name!==name);ids.push({id:`cid_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,name,type});updateCommunityUserProfile(scopeKey,{communityIds:ids,anonymousAlias:''});renderIds();};overlay.querySelectorAll('[data-id-delete]').forEach(btn=>{let timer=0;const clear=()=>{if(timer){clearTimeout(timer);timer=0;}};const remove=()=>{const ids=getCommunityUserProfile(scopeKey).communityIds.filter(x=>String(x.id)!==String(btn.dataset.idDelete));updateCommunityUserProfile(scopeKey,{communityIds:ids,anonymousAlias:''});renderIds();};btn.addEventListener('pointerdown',()=>{timer=setTimeout(remove,650)});btn.addEventListener('pointerup',clear);btn.addEventListener('pointerleave',clear);btn.addEventListener('contextmenu',e=>{e.preventDefault();remove();});});};panel.appendChild(overlay);overlay.addEventListener('click',e=>{if(e.target===overlay)overlay.remove();});renderIds();return;}
