...
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择匿名时使用的匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
  ...
  const openWeiboComposeDialog=()=>{
    ...
    overlay.innerHTML=`<div class="moli-weibo-compose-page"><div class="moli-weibo-compose-head"><button data-weibo-dialog-close>取消</button><div><b>发微博</b><small>${escapeHtml(getTavernUserContext()?.name||'User')}</small></div><button class="send" data-action="weibo-publish">发送</button></div><textarea data-weibo-compose-text placeholder="分享新鲜事…"></textarea><div class="moli-weibo-compose-images" data-weibo-compose-images hidden></div><textarea class="moli-weibo-image-input" data-weibo-image-input rows="4" placeholder="图片描述（可选）：每行一张，最多9张。例如：夜晚的校门口，路灯下站着两个人"></textarea><div class="moli-weibo-compose-tools"><button data-action="weibo-image-focus">▧<small>图片</small></button><button data-action="weibo-insert-at">@<small>提及</small></button><button data-action="weibo-insert-topic">#<small>话题</small></button></div></div>`;
    panel.appendChild(overlay);const close=()=>overlay.remove();overlay.querySelector('[data-weibo-dialog-close]').onclick=close;const body=overlay.querySelector('[data-weibo-compose-text]');const imageInput=overlay.querySelector('[data-weibo-image-input]');const refreshImages=()=>{const imgs=String(imageInput.value||'').split(/\n+/).map(x=>x.trim()).filter(Boolean).slice(0,9);const box=overlay.querySelector('[data-weibo-compose-images]');box.hidden=!imgs.length;box.innerHTML=imgs.map((x,i)=>`<div><small>图片${i+1}</small><span>${escapeHtml(x)}</span></div>`).join('');};imageInput.addEventListener('input',refreshImages);overlay.querySelector('[data-action="weibo-image-focus"]').onclick=()=>imageInput.focus();overlay.querySelector('[data-action="weibo-insert-at"]').onclick=()=>{body.setRangeText('@',body.selectionStart,body.selectionEnd,'end');body.focus();};overlay.querySelector('[data-action="weibo-insert-topic"]').onclick=()=>{body.setRangeText('##',body.selectionStart,body.selectionEnd,'end');body.selectionStart=body.selectionEnd=Math.max(0,body.selectionStart-1);body.focus();};overlay.querySelector('[data-action="weibo-publish"]').onclick=()=>{const content=String(body.value||'').trim();const images=String(imageInput.value||'').split(/\n+/).map(x=>x.trim()).filter(Boolean).slice(0,9);if(!content&&!images.length){windowRef.alert?.('写点内容或添加图片后再发送。');return;}const userName=getTavernUserContext()?.name||'User';const tags=[...content.matchAll(/#[^#\s]{1,40}#/g)].map(m=>m[0]).slice(0,12);createPublicWebPost(getScopeKey?.(),{section:'weibo',type:'weibo',author:{type:'user',id:'user',name:userName},title:content.slice(0,40),content,tags,extra:{weiboLane:'关注',images,userOwned:true,reposts:0,hotScore:0}});close();renderPublicWeb();};body.focus();
  };
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event => {
