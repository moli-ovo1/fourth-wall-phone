...
import { notifyMomentInteractionOpportunity, notifyBehaviorOpportunity, notifyBehaviorContextEvent } from '../automation/private-automation.js?v=server-community-trial-4';
...
export function createPhonePanel({
  ...
  panel.innerHTML = `
    ...
    <section class="moli-page" data-page="chat">
      ...
      <div class="moli-studio-after-adopt" data-studio-after-adopt hidden>
        <button type="button" data-studio-after-adopt="clear">清空本次讨论</button>
        <button type="button" data-studio-after-adopt="continue">我还没完呢</button>
      </div>
  ...
  const displayName = item => {
    ...
    const base = item?.remark || item?.displayName || item?.name || item?.source?.originalName || '未命名';
    return item?.contactOrigin === 'network' ? `${base}（网友）` : base;
  };
  ...
  function focusStudioInput(task) {
    ...
    input.classList.add('moli-studio-input-flash');
    input.focus();
    windowRef.setTimeout(() => input.classList.remove('moli-studio-input-flash'), 1350);
  }
  ...
  function captureExplicitChatAnonymousDisclosure(scopeKey, conversation, text) {
    ...
    if (!/(?:那个|这个|帖子里|评论里|回答里|社区里)?\s*(?:小号(?:的|那个|用户|账号|人)?|小号者).{0,10}(?:是我|就是我|我发的|我写的|我本人)|(?:是我|就是我|我发的|我写的).{0,10}(?:小号(?:的|那个|用户|账号|人)?|小号者)/u.test(content)) return 0;
    ...
    for (const message of forwards) {
      ...
      // Deictic “那个小号的是我” is only safe when the active recent Community
    ...
    const event = recordWorldEvent(scopeKey, {
      ...
      content: `User 在微信中明确告诉你：刚才相关社区内容里的小号身份“${candidate.alias}”就是 User 本人。`,
      metadata: {
    ...
    try {
      ...
      if (isWritersRoom(conversation)) {
        if (/^(?:采纳|取纳)(?:\s|[，,：:。！!？?]|$)/u.test(text)) {
          ...
          windowRef.setTimeout(() => requestReply({ studioTask: task }), 0);
        }
      }
    } catch (error) {
  ...
  const communityDiscussionContext = post => {
    ...
    if(post?.section==='zhihu'){
      for(const answer of (post?.extra?.answers||[]).slice(-8)){
        rows.push(`回答｜${answer?.author?.name||'小号用户'}：${answer?.content||''}`);
  ...
  const communitySelfAuthoredContext = (post, contactId) => {
    ...
    const surface=author=>author?.anonymous?`小号账号“${String(author?.name||'小号用户')}”`:`本人账号“${String(author?.name||'')}”`;
  ...
  const openCommunityComposer = ({postId, replyToCommentId='', zhihuAnswerId='', replyLabel='', composeMode='comment'}) => {
    ...
    const modal=panel.querySelector('[data-community-composer]'); communityComposerState.identityMode='real'; modal?.classList.toggle('is-weibo-composer',post.section==='weibo');
    const identityButton=modal?.querySelector('[data-community-identity-button]'); if(identityButton)identityButton.textContent='大号';
    const alias=modal?.querySelector('[data-community-alias]'); if(alias)alias.value=getAnonymousAlias(postId)||'小号用户';
  ...
  const closeCommunityComposer=()=>{const modal=panel.querySelector('[data-community-composer]');modal?.classList.remove('is-open','is-weibo-composer');communityComposerState=null;};
  ...
  const settleQueuedCommunityTargets = async post => {
    const pending=pendingForPost(post?.id); const targeted=pending.filter(x=>x.type==='invite'||x.type==='mention'); const passerbyMentions=pending.filter(x=>x.type==='passerby-mention'); if(!targeted.length&&!passerbyMentions.length)return [];
    const done=[]; const notices=[];
    for(const item of passerbyMentions){
      try{const actor=item.actor||{};const reply=await generateCommunityPasserbyMentionReply({scopeKey:getScopeKey?.(),post,actor,userContent:item.content,threadContext:passerbyThreadContext(post,actor)});if(post.section==='zhihu'&&item.answerId)addZhihuAnswerComments(getScopeKey?.(),post.id,item.answerId,[{...reply,replyToCommentId:String(item.replyToCommentId||'')}]);else addPublicWebComment(getScopeKey?.(),post.id,{...reply,replyToCommentId:String(item.replyToCommentId||'')});done.push(item.id);notices.push(`${String(actor.name||'网友')}已回应 @`);}catch(error){console.error('[moli小手机] passerby mention failed:',error);throw error;}
    }
...
${continuity?`【你自己的手机经历/认知】\n${continuity}\n`:''}${item.type==='invite'?`User 明确邀请你${isAnswer?'回答这个问题':'参与跟帖'}`:`User 在评论中明确 @了你：${item.content||''}`}。这是 User 的明确互动，你必须${isAnswer?'回答':'回应'}，不允许 SKIP。${forcedMode==='real'?'User @的是你的本名，本次必须使用本名公开回应。':forcedMode==='surface'&&forcedAlias?`User @的是你在本帖已经使用的公开账号“${forcedAlias}”，本次必须继续使用“${forcedAlias}”回应，不得切换为本名，也不得暴露该账号背后的真实身份。`:'你只判断使用 REPLY_REAL（实名）还是 REPLY_ANONYMOUS（小号）；小号表示使用小号网名公开回应。'}${proactiveEnabled?'公开回应完成后，再独立判断是否需要私聊 User；SEND 时给 2~5 条真实手机气泡，SKIP 时可给简短原因。':'主动私聊权限关闭：PRIVATE 必须 SKIP。'}
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择小号时使用的小号网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
      try{
        const result=await generatePrivateReply({scopeKey,conversationKey,automationInstruction:instruction,allowNoPendingUser:true}); const raw=String(result?.text||'');
        ...
        const legacyIdentity=(raw.match(/<community_identity>\s*(REAL|ANONYMOUS)\s*<\/community_identity>/i)?.[1]||'REAL').toUpperCase(); let identity=decision==='REPLY_ANONYMOUS'?'ANONYMOUS':decision==='REPLY_REAL'?'REAL':legacyIdentity; let alias=String(raw.match(/<community_alias>([\s\S]*?)<\/community_alias>/i)?.[1]||'').trim()||'小号用户'; if(forcedMode==='real')identity='REAL'; if(forcedMode==='surface'&&forcedAlias){identity='ANONYMOUS';alias=forcedAlias;} const author=identity==='ANONYMOUS'?{type:'contact',id:target.id,name:alias,uiName:`${alias}（${displayName(target)}）`,anonymous:true,knownIdentityId:target.id,identityKnownBy:[target.id]}:{type:'contact',id:target.id,name:displayName(target),anonymous:false};
        let publicDid=false,publicText='';if(isAnswer){const text=String(raw.match(/<community_answer>([\s\S]*?)<\/community_answer>/i)?.[1]||'').trim();if(text){publicText=text;addZhihuAnswer(scopeKey,post.id,{author,content:text});publicDid=true;}}else{const text=String(raw.match(/<community_reply>([\s\S]*?)<\/community_reply>/i)?.[1]||'').trim();if(text){publicText=text;if(post.section==='zhihu'&&item.answerId)addZhihuAnswerComments(scopeKey,post.id,item.answerId,[{author,content:text,replyToCommentId:String(item.replyToCommentId||'')}]);else addPublicWebComment(scopeKey,post.id,{author,content:text,replyToCommentId:String(item.replyToCommentId||'')});publicDid=true;}}
        if(!publicDid) throw new Error(`角色必须${isAnswer?'回答':'回应'}，但模型没有返回可写入的公开正文，请重试刷新。`);
        const privateDecision=proactiveEnabled?(raw.match(/<community_private>\s*(SEND|SKIP)\s*<\/community_private>/i)?.[1]||'SKIP').toUpperCase():'SKIP'; const privateReason=proactiveEnabled?String(raw.match(/<community_private_reason>([\s\S]*?)<\/community_private_reason>/i)?.[1]||'此刻没有需要单独私聊的动机。').trim():'主动私聊权限未开启'; const msgs=[...raw.matchAll(/<msg>([\s\S]*?)<\/msg>/gi)].map(m=>String(m[1]||'').trim()).filter(Boolean).slice(0,5);let privateDid=false;if(privateDecision==='SEND'&&msgs.length){for(const text of msgs)appendMessage(scopeKey,conversationKey,'assistant',text,{source:'community-decision',senderId:target.id,senderSnapshot:{name:displayName(target),avatar:avatarUrl(target)}});privateDid=true;}
        const resultEvent=recordWorldEvent(scopeKey,{source:`community.${post.section||'unknown'}`,actorId:target.id,action:isAnswer?'PUBLIC_ANSWER':'PUBLIC_REPLY',targetContactIds:[target.id],objectId:String(post.id||''),content:`你在${sourceLabel(post)}公开${isAnswer?'回答':'回应'}了 User 的明确互动：“${publicText.slice(0,500)}”`,metadata:{postId:String(post.id||''),pendingId:String(item.id||''),identityMode:identity,publicDecision:'REPLY',publicContent:publicText,anonymousAlias:identity==='ANONYMOUS'?alias:'',privateDecision:privateDid?'SEND':'SKIP',privateReason,privateMessages:privateDid?msgs:[]},awareness:'known',dedupeKey:`community-role:${item.id}`});
        ...
        markWorldEventsConsumedByObjectTargets(scopeKey,[item.id],'community-settlement'); done.push(item.id); notices.push(`${displayName(target)}${identity==='ANONYMOUS'?'已小号':'已实名'}${isAnswer?'回答':'回应'}；${privateDid?'已私聊':`未私聊：${privateReason}`}`);
  ...
  const runCommunityMention = async ({post,userEntry,mentionTarget,replyToCommentId='',zhihuAnswerId=''}) => {
    ...
    try{
      const platform=sourceLabel(post); const instruction=`【moli社区事件｜${platform} @提及】\nUser 在${platform}的一条内容下 @了你。\n帖子/问题：${post.title||'无标题'}\n正文：${post.content||'无'}\nUser 的评论：${userEntry.content}\n这是 User 对你的明确 @，你必须公开跟帖回应。你只在实名与小号之间选择，不允许 SKIP；小号表示使用小号网名公开回应。你可以自行决定是否另外私聊 User，但公开回复不可省略。\n严格追加机器可读块：<community_action>REPLY|BOTH</community_action><community_identity>REAL|ANONYMOUS</community_identity><community_alias>小号时使用的网名</community_alias>；公开回复追加 <community_reply>公开回复正文</community_reply>；若私聊，再用正常 <msg>私聊内容</msg>。公开回复只能以你自己的身份发言，绝不能代替 User。`;
      const result=await generatePrivateReply({scopeKey,conversationKey,automationInstruction:instruction,allowNoPendingUser:true});
      ...
      const publicReply=String(raw.match(/<community_reply>([\s\S]*?)<\/community_reply>/i)?.[1]||'').trim(); const identity=(raw.match(/<community_identity>\s*(REAL|ANONYMOUS)\s*<\/community_identity>/i)?.[1]||'REAL').toUpperCase(); const alias=String(raw.match(/<community_alias>([\s\S]*?)<\/community_alias>/i)?.[1]||'').trim()||'小号用户';
      const msgs=[...raw.matchAll(/<msg>([\s\S]*?)<\/msg>/gi)].map(m=>String(m[1]||'').trim()).filter(Boolean);
  ...
  const settleRefreshedCharacterPosts = items => {
    const scopeKey=getScopeKey?.();
    for(const batch of Array.isArray(items?.characterPostEcology)?items.characterPostEcology:[]){
      settleCharacterPostEcology(scopeKey,batch?.postId,batch);
    }
  };
  const publicWebPostsForTab = () => {
    if (currentPublicWebTab === 'recommend') {
      ...
      return all.filter(post => post.author?.type === 'character' || (batchId && String(post.extra?.recommendationBatchId || '') === batchId));
    }
  ...
  const renderPublicWebDetail = post => {
    ...
    const comments = Array.isArray(post.comments) ? post.comments : [];
    const author = escapeHtml(post.author?.name || '小号网友');
    ...
    if (post.section === 'weibo') {
      const name=communityAuthorDisplay(post.author); const aid=String(post.author?.id||post.author?.name||'');
      const isFollowed=isWeiboFollowed(getScopeKey?.(),aid)||isWeiboFollowed(getScopeKey?.(),name);
      const imgs=(Array.isArray(post.extra?.images)?post.extra.images:[post.extra?.imagePrompt].filter(Boolean)).slice(0,9);
      const vids=(Array.isArray(post.extra?.videos)?post.extra.videos:[]).slice(0,1);
      const media=(imgs.length||vids.length)?`<div class="moli-weibo-media moli-weibo-media-${Math.min(Math.max(imgs.length,vids.length),9)}">${imgs.map(x=>`<div><span>图片：（${escapeHtml(x)}）</span></div>`).join('')}${vids.map(x=>`<div class="is-video"><span>视频：（${escapeHtml(x)}）</span></div>`).join('')}</div>`:'';
      const byId=new Map(comments.map(c=>[String(c.id),c]));
      const rootOf=c=>{let cur=c,guard=0;while(cur?.replyToCommentId&&byId.has(String(cur.replyToCommentId))&&guard++<20)cur=byId.get(String(cur.replyToCommentId));return cur;};
      const top=comments.filter(c=>!c.replyToCommentId||!byId.has(String(c.replyToCommentId)));
      const avatar=a=>`<span class="moli-weibo-comment-avatar">${escapeHtml(Array.from(communityAuthorDisplay(a)||'微')[0]||'微')}</span>`;
      const repostIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 7h8l-2.5-2.5M16 7l-2.5 2.5M16 17H8l2.5 2.5M8 17l2.5-2.5"/></svg>';
      const replyIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 5h14v10H9l-4 4V5z"/></svg>';
      const likeIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 10v9H4v-9h3zm3 9h7.5c1 0 1.8-.7 2-1.6l1.2-5.4c.2-1.1-.6-2-1.7-2h-4l.7-3.1C16 5.4 15 4 13.6 4L9 10v9z"/></svg>';
      const row=(x,nested=false)=>{const owner=isPostOwnerAuthor(post,x.author)?'<span class="moli-weibo-owner-badge">博主</span>':'';const target=byId.get(String(x.replyToCommentId||''));return `<div class="moli-weibo-comment${nested?' is-reply':''}" data-community-comment="1" data-post-id="${escapeHtml(post.id)}" data-comment-id="${escapeHtml(x.id)}">${avatar(x.author)}<div class="moli-weibo-comment-main"><div class="moli-weibo-comment-name">${escapeHtml(communityAuthorDisplay(x.author))}${owner}</div><p>${target?`回复 @${escapeHtml(communityAuthorDisplay(target.author))}：`:''}${escapeHtml(x.content||'')}</p><div class="moli-weibo-comment-actions"><button data-action="weibo-comment-repost" data-post-id="${escapeHtml(post.id)}" data-comment-id="${escapeHtml(x.id)}" aria-label="转发">${repostIcon}</button><button data-action="weibo-comment-reply" data-post-id="${escapeHtml(post.id)}" data-comment-id="${escapeHtml(x.id)}" data-comment-author="${escapeHtml(communityAuthorDisplay(x.author))}" aria-label="回复">${replyIcon}</button><button data-action="weibo-like" data-post-id="${escapeHtml(post.id)}" aria-label="赞">${likeIcon}</button></div></div></div>`;};
      const commentRows=top.map(c=>{const replies=comments.filter(r=>String(r.id)!==String(c.id)&&String(rootOf(r)?.id||'')===String(c.id));const expanded=expandedWeiboThreads.has(String(c.id));const shown=expanded?replies:replies.slice(0,1);const more=replies.length>1?`<button class="moli-weibo-expand-replies" data-action="weibo-toggle-replies" data-comment-id="${escapeHtml(c.id)}">${expanded?'收起回复':`……共${replies.length}条回复 >`}</button>`:'';return `<div class="moli-weibo-comment-thread">${row(c)}${shown.map(r=>row(r,true)).join('')}${more}</div>`;}).join('');
      const repostAuthor=post.extra?.repostAuthor?.name||String(post.extra?.repostText||'').match(/@([^：:\n]{1,40})[：:]/)?.[1]||String(post.extra?.repostChain?.[0]||'').match(/@([^：:\n]{1,40})[：:]/)?.[1]||'';
      const repostBlock=post.extra?.repostText?`<div class="moli-weibo-repost">${repostAuthor?`<div class="moli-weibo-repost-author"><span class="moli-weibo-avatar">${escapeHtml(Array.from(repostAuthor)[0]||'微')}</span><b>${escapeHtml(repostAuthor)}</b></div>`:''}<div>${escapeHtml(post.extra.repostText)}</div>${(post.extra?.repostChain||[]).map(x=>`<div>${escapeHtml(x)}</div>`).join('')}</div>`:'';
      feed.innerHTML=`<article class="moli-weibo-detail"><div class="moli-weibo-detail-head"><button data-action="public-web-detail-back">‹</button><b>微博正文</b><span></span></div><div class="moli-weibo-detail-author"><span class="moli-weibo-avatar">${escapeHtml(Array.from(name||'微')[0]||'微')}</span><div><b>${escapeHtml(name)}</b><small>${new Date(Number(post.createdAt||Date.now())).toLocaleString()}</small></div><button data-action="weibo-follow" data-account-id="${escapeHtml(aid)}" data-account-name="${escapeHtml(name)}">${isFollowed?'已关注':'+关注'}</button></div><div class="moli-weibo-detail-body">${(post.tags||[]).map(t=>`<span class="moli-weibo-tag">${escapeHtml(t)}</span>`).join(' ')}<p>${escapeHtml(post.content||'').replace(/\n/g,'<br>')}</p>${media}${repostBlock}</div><div class="moli-weibo-detail-tools">${communityActions}</div><div class="moli-weibo-detail-tabs"><button data-action="weibo-repost" data-post-id="${escapeHtml(post.id)}">转发 ${Number(post.extra?.reposts||0)}</button><button data-action="public-web-comment" data-post-id="${escapeHtml(post.id)}">评论 ${comments.length}</button><button data-action="weibo-like" data-post-id="${escapeHtml(post.id)}">赞 ${(post.extra?.likes||[]).length}</button></div><div class="moli-weibo-comments">${commentRows||'<div class="moli-web-muted">暂无评论</div>'}</div></article>`;
    } else if (post.section === 'custom') {
  ...
  const expandedWeiboThreads=new Set();
  ...
  const renderPublicWeb = () => {
    ...
    if (tianyaChrome) {
      ...
      feed.innerHTML=`<div class="moli-custom-home">
        <section class="moli-custom-user-home"><button type="button" class="moli-custom-section-head moli-custom-user-home-head" data-action="custom-user-home-toggle"><h3>${escapeHtml(userName)}主页</h3><span>›</span></button><div class="moli-custom-user-home-body" hidden><button type="button" class="moli-community-id-manager" data-action="community-id-manager">ID card</button><div class="moli-community-id-inline" data-community-id-inline hidden></div></div></section>
      ...
      const mediaFor=post=>{const imgs=(Array.isArray(post.extra?.images)?post.extra.images:[post.extra?.imagePrompt].filter(Boolean)).slice(0,9),vids=(Array.isArray(post.extra?.videos)?post.extra.videos:[]).slice(0,4);const n=Math.max(imgs.length,vids.length);return n?`<div class="moli-weibo-media moli-weibo-media-${Math.min(n,9)}">${imgs.map(x=>`<div><span>图片：（${escapeHtml(x)}）</span></div>`).join('')}${vids.map(x=>`<div class="is-video"><span>视频：（${escapeHtml(x)}）</span></div>`).join('')}</div>`:'';};
      const previewFor=post=>{const raw=String(post.content||post.title||'');const long=raw.length>150;return `${escapeHtml(long?raw.slice(0,150):raw)}${long?'<span class="moli-weibo-fulltext">…正文</span>':''}`;};
      const weiboRelativeTime=value=>{const ts=Number(value||0);if(!ts)return '刚刚';const diff=Math.max(0,Date.now()-ts);if(diff<60*1000)return '刚刚';if(diff<60*60*1000)return `${Math.max(1,Math.floor(diff/60000))}分钟前`;if(diff<24*60*60*1000)return `${Math.max(1,Math.floor(diff/3600000))}小时前`;if(diff<7*24*60*60*1000)return `${Math.max(1,Math.floor(diff/86400000))}天前`;const d=new Date(ts);return `${d.getMonth()+1}-${String(d.getDate()).padStart(2,'0')} ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;};
      const card=post=>{const aid=String(post.author?.id||post.author?.name||'');const name=communityAuthorDisplay(post.author);const isFollowed=followed.has(aid)||followed.has(name);const tags=(post.tags||[]).map(t=>`<span class="moli-weibo-tag">${escapeHtml(t)}</span>`).join(' ');const chain=(post.extra?.repostChain||[]).map(x=>`<div>${escapeHtml(x)}</div>`).join('');return `<article class="moli-weibo-card"><header>${avatarFor(name)}<div><b>${escapeHtml(name)}</b><small>${escapeHtml(post.extra?.weiboLane||'实时')} · ${escapeHtml(weiboRelativeTime(post.createdAt))}</small></div><button type="button" data-action="weibo-follow" data-account-id="${escapeHtml(aid)}" data-account-name="${escapeHtml(name)}">${isFollowed?'已关注':'+关注'}</button></header><button class="moli-weibo-open" data-action="public-web-open" data-post-id="${escapeHtml(post.id)}"><p>${tags?tags+' ':''}${previewFor(post)}</p>${mediaFor(post)}${post.extra?.repostText?`<div class="moli-weibo-repost">${(()=>{const rn=post.extra?.repostAuthor?.name||String(post.extra.repostText||'').match(/@([^：:\n]{1,40})[：:]/)?.[1]||String(post.extra?.repostChain?.[0]||'').match(/@([^：:\n]{1,40})[：:]/)?.[1]||'';return rn?`<div class="moli-weibo-repost-author"><span class="moli-weibo-avatar">${escapeHtml(Array.from(rn)[0]||'微')}</span><b>${escapeHtml(rn)}</b></div>`:'';})()}<div>${escapeHtml(post.extra.repostText)}</div>${chain}</div>`:''}</button><footer><span>转发 ${Number(post.extra?.reposts||0)}</span><span>评论 ${(post.comments||[]).length}</span><span>赞 ${(post.extra?.likes||[]).length}</span></footer></article>`;};
      const hot=posts.filter(p=>String(p.extra?.weiboLane||'')==='热门').slice().sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0)); const hotTopics=getWeiboHotTopics(getScopeKey?.()); const pinnedWeibo=posts.filter(p=>Boolean(p.extra?.pinned));
      feed.innerHTML=`<div class="moli-weibo-home"><div class="moli-weibo-subnav"><span class="moli-weibo-nav-spacer"></span><div class="moli-weibo-nav-center"><button class="${currentWeiboView==='pinned'?'is-active':''}" data-weibo-view="pinned">常驻</button><button class="${currentWeiboView==='home'?'is-active':''}" data-weibo-view="home">首页</button><button class="${currentWeiboView==='supertopic'?'is-active':''}" data-weibo-view="supertopic">超话</button><button class="${currentWeiboView==='hot'?'is-active':''}" data-weibo-view="hot">热搜</button></div><div class="moli-weibo-nav-actions"><button data-action="weibo-messages" aria-label="私信">✉</button><button class="moli-weibo-refresh${publicWebGenerating.has('weibo')?' is-spinning':''}" data-action="public-web-refresh" aria-label="刷新">↻</button><button class="moli-weibo-compose" data-action="weibo-compose" aria-label="发微博">＋</button></div></div><div data-weibo-pinned ${currentWeiboView==='pinned'?'':'hidden'}>${pinnedWeibo.map(card).join('')||'<div class="moli-recommend-empty">还没有常驻微博。</div>'}</div><div data-weibo-home ${currentWeiboView==='home'?'':'hidden'}>${posts.filter(p=>!p.extra?.pinned&&!['超话','热门'].includes(String(p.extra?.weiboLane||''))).map(card).join('')||'<div class="moli-recommend-empty">点一下 ↻，看看微博正在发生什么。</div>'}</div><div data-weibo-supertopic ${currentWeiboView==='supertopic'?'':'hidden'}>${posts.filter(p=>!p.extra?.pinned&&String(p.extra?.weiboLane||'')==='超话').map(card).join('')||'<div class="moli-recommend-empty">点一下 ↻，生成当前世界的CP超话。</div>'}</div><div data-weibo-hot ${currentWeiboView==='hot'?'':'hidden'}><button class="moli-weibo-search" data-action="weibo-search">⌕ <span>搜索微博 / 添加关注人</span></button><section class="moli-weibo-hot-list"><h3>微博热搜</h3>${hotTopics.map((topic,i)=>`<div class="moli-weibo-hot-keyword"><b>${i+1}</b><span>${escapeHtml(topic)}</span></div>`).join('')||'<p>刷新后生成当前世界的热搜。</p>'}</section><h3 class="moli-weibo-hot-title">热门微博</h3>${hot.map(card).join('')||'<div class="moli-recommend-empty">搜索关键词后，相关微博会出现在这里。</div>'}</div></div>`;
  ...
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event=>{
    const scope=getScopeKey?.();
    const userHomeToggle=event.target.closest('[data-action="custom-user-home-toggle"]');if(userHomeToggle){const body=panel.querySelector('.moli-custom-user-home-body');if(body)body.hidden=!body.hidden;return;} const idManager=event.target.closest('[data-action="community-id-manager"]');if(idManager){const scopeKey=getScopeKey?.();const box=panel.querySelector('[data-community-id-inline]');if(!box)return;const renderIds=()=>{const profile=getCommunityUserProfile(scopeKey);box.innerHTML=`<div class="moli-community-id-add"><select data-id-type><option>大号</option><option selected>小号</option></select><input data-id-name placeholder="输入社区ID"><button data-id-add>保存</button></div><div class="moli-community-id-list">${profile.communityIds.map(x=>`<button data-id-delete="${escapeHtml(x.id)}">（${escapeHtml(x.type)}）${escapeHtml(x.name)}</button>`).join('')||'<p class="moli-web-muted">还没有保存社区ID。</p>'}</div><p class="moli-community-id-delete-tip">长按名字可删除</p>`;box.querySelector('[data-id-add]').onclick=()=>{const name=String(box.querySelector('[data-id-name]')?.value||'').trim();if(!name)return;const type=String(box.querySelector('[data-id-type]')?.value||'小号');const ids=getCommunityUserProfile(scopeKey).communityIds.filter(x=>x.name!==name);ids.push({id:`cid_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,name,type});updateCommunityUserProfile(scopeKey,{communityIds:ids,anonymousAlias:''});renderIds();};box.querySelectorAll('[data-id-delete]').forEach(btn=>{let timer=0;const clear=()=>{if(timer){clearTimeout(timer);timer=0;}};const remove=()=>{const ids=getCommunityUserProfile(scopeKey).communityIds.filter(x=>String(x.id)!==String(btn.dataset.idDelete));updateCommunityUserProfile(scopeKey,{communityIds:ids,anonymousAlias:''});renderIds();};btn.addEventListener('pointerdown',()=>{timer=setTimeout(remove,650)});btn.addEventListener('pointerup',clear);btn.addEventListener('pointerleave',clear);btn.addEventListener('contextmenu',e=>{e.preventDefault();remove();});});};box.hidden=!box.hidden;if(!box.hidden)renderIds();return;} const todayToggle=event.target.closest('[data-action="recommend-today-toggle"]'); if(todayToggle){recommendTodayOpen=!recommendTodayOpen;renderPublicWeb();return;}
  ...
  panel.querySelector('[data-action="public-web-refresh"]')?.addEventListener('click', async event => {
    ...
    try {
      ...
      if (currentPublicWebTab === 'recommend') {
        ...
        const items=await generatePublicWebRefresh({scopeKey:getScopeKey?.(),ghostStoriesEnabled:settings.ghostStoriesEnabled,fixedPersonasCommunityEnabled:settings.fixedPersonasCommunityEnabled,section:'recommend',recommendSources:settings.recommendCustomized?settings.recommendSources:null,recommendCount:settings.recommendCustomized?settings.recommendCount:0,customCommunities:listCustomCommunities(getScopeKey?.()).filter(x=>!Array.isArray(settings.recommendCustomIds)||!settings.recommendCustomIds.length||settings.recommendCustomIds.includes(x.id))});
        settleRefreshedCharacterPosts(items);
        const createdPosts=addPublicWebPosts(getScopeKey?.(),items.map(item=>({ ...item, extra:{ ...(item.extra||{}), recommendationBatchId:batchId } })));
        await settleCommunityDiscovery(createdPosts,settings);
        updatePublicWebSettings(getScopeKey?.(),{ recommendationBatchId:batchId });
      } else {
        const items=await generatePublicWebRefresh({scopeKey:getScopeKey?.(),ghostStoriesEnabled:settings.ghostStoriesEnabled,fixedPersonasCommunityEnabled:settings.fixedPersonasCommunityEnabled,section:currentPublicWebTab});
        settleRefreshedCharacterPosts(items);
        if(currentPublicWebTab==='weibo'){const createdPosts=appendPublicWebSectionPosts(getScopeKey?.(),'weibo',items);await settleCommunityDiscovery(createdPosts,settings);trimWeiboLanePosts(getScopeKey?.(),15);for(const item of items){const message=String(item?.extra?.privateMessage||'').trim();const id=String(item?.author?.id||item?.author?.name||'').trim();const name=String(item?.author?.name||id).trim();if(message&&id&&name){saveWeiboMessagePeer(getScopeKey?.(),{id,name,profile:'由微博公开互动产生的持续网络账号'});addWeiboPrivateMessage(getScopeKey?.(),id,{role:'account',content:message});}}}else{const createdPosts=replacePublicWebSectionPosts(getScopeKey?.(),currentPublicWebTab,items);await settleCommunityDiscovery(createdPosts,settings);}
      }
  ...
  panel.querySelector('[data-public-web-feed]')?.addEventListener('contextmenu', event => {
    ...
    const answerTarget=event.target?.closest?.('[data-community-answer="1"]');
    if(answerTarget){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation?.();const postId=String(answerTarget.dataset.postId||'');const answerId=String(answerTarget.dataset.answerId||'');if(!postId||!answerId)return;if(windowRef.confirm?.('删除这条知乎回答？')){const result=deleteZhihuAnswer(getScopeKey?.(),postId,answerId);if(!result?.ok)windowRef.alert?.('没有找到这条回答，可能已经被删除。');renderPublicWeb();}return;}
  ...
  const openWeiboSearchDialog=()=>{
    const overlay=document.createElement('div');overlay.className='moli-weibo-overlay';
    overlay.innerHTML=`<div class="moli-weibo-search-dialog"><div class="moli-weibo-dialog-head"><b>搜索 / 添加关注人</b><button data-weibo-dialog-close>×</button></div><div class="moli-weibo-search-note"><b>搜索想看的关键词</b><br><br><b>自创关注人，示例：</b><br>ID：娱乐圈搅屎棍<br>身份：大V（可不填）<br>人设：娱乐圈著名狗仔，说话辛辣，爱在评论区和粉丝吵架<br>语言风格：你再嘚嘚，你家哥哥难保<br><br>ID：一只喇叭花<br>人设：学校里的大喇叭，超爱传八卦<br><br><small>身份可选填：普通用户 / 大V / 媒体 / 官号 / 营销号 / 博主 / 狗仔 / 粉丝站 / 校园墙 / 行业从业者 / 知情人。它只是账号公开定位，不填也可以。</small></div><textarea data-weibo-search-input rows="7" placeholder="搜索时直接写关键词；添加关注人时按上面的格式填写"></textarea><div class="moli-weibo-dialog-actions"><button data-action="weibo-search-submit">搜索</button><button class="primary" data-action="weibo-follow-create">添加关注人</button></div></div>`;
    panel.appendChild(overlay);const close=()=>overlay.remove();overlay.querySelector('[data-weibo-dialog-close]').onclick=close;overlay.addEventListener('click',e=>{if(e.target===overlay)close();});
    overlay.querySelector('[data-action="weibo-search-submit"]').onclick=async()=>{const q=String(overlay.querySelector('[data-weibo-search-input]')?.value||'').trim();if(!q)return;const btn=overlay.querySelector('[data-action="weibo-search-submit"]');btn.disabled=true;btn.textContent='搜索中…';try{const items=await generatePublicWebRefresh({scopeKey:getScopeKey?.(),fixedPersonasCommunityEnabled:getPublicWebSettings(getScopeKey?.()).fixedPersonasCommunityEnabled,section:'weibo',weiboQuery:q,weiboMode:'search'});const createdPosts=appendPublicWebSectionPosts(getScopeKey?.(),'weibo',items);await settleCommunityDiscovery(createdPosts,getPublicWebSettings(getScopeKey?.()));close();renderPublicWeb();const hotBtn=panel.querySelector('[data-weibo-view="hot"]');hotBtn?.click();}catch(error){windowRef.alert?.(`搜索失败：${error?.message||error}`);btn.disabled=false;btn.textContent='搜索';}};
    overlay.querySelector('[data-action="weibo-follow-create"]').onclick=()=>{const raw=String(overlay.querySelector('[data-weibo-search-input]')?.value||'').trim();const pick=label=>String(raw.match(new RegExp(`(?:^|\\n)\\s*${label}\\s*[：:]\\s*([^\\n]+)`,'i'))?.[1]||'').trim();const name=pick('ID');if(!name){windowRef.alert?.('添加关注人时请至少填写 ID。');return;}const identity=pick('身份'),persona=pick('人设'),voice=pick('语言风格');const profile=[identity&&`身份：${identity}`,persona&&`人设：${persona}`,voice&&`语言风格：${voice}`].filter(Boolean).join('；');saveWeiboFollow(getScopeKey?.(),{id:name,name,profile,source:'manual'});close();renderPublicWeb();};
  };
  const openWeiboMessagesDialog=()=>{
    const scopeKey=getScopeKey?.(); const overlay=document.createElement('div'); overlay.className='moli-weibo-overlay moli-weibo-network-overlay'; let tab='messages';
    const follows=()=>listWeiboFollows(scopeKey).sort((a,b)=>Number(Boolean(b.hot))-Number(Boolean(a.hot))||Number(b.updatedAt||0)-Number(a.updatedAt||0));
    const avatar=x=>x?.avatar?`<img src="${escapeHtml(x.avatar)}" alt="">`:`<span>${escapeHtml(Array.from(x?.name||'微')[0]||'微')}</span>`;
    const userName=()=>String(getTavernUserContext()?.name||'User');
    const notifications=()=>{const posts=listPublicWebPosts(scopeKey,{section:'weibo'});const mine=[];const replies=[];for(const post of posts){const comments=Array.isArray(post.comments)?post.comments:[];const myIds=new Set(comments.filter(c=>c.author?.type==='user'||String(c.author?.id)==='user').map(c=>String(c.id)));for(const c of comments){if(c.author?.type==='user'||String(c.author?.id)==='user')continue;const text=String(c.content||'');const row={postId:post.id,author:communityAuthorDisplay(c.author),content:text,createdAt:Number(c.createdAt||post.createdAt||0)};if(text.includes(`@${userName()}`)||text.includes('@User'))mine.push(row);if(myIds.has(String(c.replyToCommentId||'')))replies.push(row);}}return{mentions:mine.sort((a,b)=>b.createdAt-a.createdAt),comments:replies.sort((a,b)=>b.createdAt-a.createdAt)};};
    const unread=()=>{const read=getWeiboMessageReadState(scopeKey),note=notifications();const dm=follows().reduce((n,x)=>n+listWeiboPrivateMessages(scopeKey,x.id).filter(m=>m.role==='account'&&Number(m.createdAt||0)>Number(read[`dm:${x.id}`]||0)).length,0);return{mentions:note.mentions.filter(x=>x.createdAt>Number(read.mentions||0)).length,comments:note.comments.filter(x=>x.createdAt>Number(read.comments||0)).length,dm};};
    const shell=(body,title='消息',options={})=>{const u=unread();const root=options.root!==false;overlay.innerHTML=`<div class="moli-weibo-network-page">${root?`<div class="moli-weibo-network-top"><button data-net-close aria-label="返回">‹</button><div class="moli-weibo-network-tabs"><button data-net-tab="follows" class="${tab==='follows'?'is-active':''}">关注人</button><button data-net-tab="messages" class="${tab==='messages'?'is-active':''}">消息${u.mentions+u.comments+u.dm?`<i>${u.mentions+u.comments+u.dm}</i>`:''}</button></div><button data-net-add aria-label="新建粉丝群">⊕</button></div>`:`<div class="moli-weibo-network-head moli-weibo-network-subhead"><button data-net-back>‹</button><b>${escapeHtml(title)}</b><span></span></div>`}${body}</div>`;overlay.querySelector('[data-net-close]')?.addEventListener('click',()=>overlay.remove());overlay.querySelector('[data-net-back]')?.addEventListener('click',renderRoot);overlay.querySelector('[data-net-add]')?.addEventListener('click',openGroupCreate);overlay.querySelectorAll('[data-net-tab]').forEach(b=>b.onclick=()=>{tab=b.dataset.netTab;renderRoot();});};
    const renderRoot=()=>{if(tab==='follows')renderFollows();else renderMessages();};
    const renderFollows=()=>{const rows=follows();shell(`<div class="moli-weibo-network-search">⌕ 搜索关注人</div><div class="moli-weibo-network-list">${rows.map(x=>`<div class="moli-weibo-network-row"><button class="moli-weibo-network-person" data-follow-card="${escapeHtml(x.id)}"><span class="moli-weibo-network-avatar">${avatar(x)}</span><span><b>${escapeHtml(x.name)} ${x.hot?'🔥':''}</b><small>${escapeHtml(x.profile||'暂无账号画像')}</small></span></button><button class="moli-weibo-unfollow" data-unfollow="${escapeHtml(x.id)}">已关注</button></div>`).join('')||'<p class="moli-web-muted">还没有关注人。</p>'}</div>`,'关注人');overlay.querySelectorAll('[data-follow-card]').forEach(b=>b.onclick=()=>renderProfile(b.dataset.followCard));overlay.querySelectorAll('[data-unfollow]').forEach(b=>b.onclick=()=>{const row=rows.find(x=>String(x.id)===String(b.dataset.unfollow));if(row&&windowRef.confirm?.(`确定取关 @${row.name}？\n\n取关会删除这个关注人的持续认知记忆和私信记录，之后重新关注不会自动恢复。`)){deleteWeiboFollow(scopeKey,row.id);renderFollows();}});};
    const renderProfile=id=>{const a=follows().find(x=>String(x.id)===String(id));if(!a)return renderFollows();shell(`<div class="moli-weibo-profile-card"><button class="moli-weibo-profile-avatar" data-profile-avatar>${avatar(a)}<small>更换头像</small></button><h3>${escapeHtml(a.name)} ${a.hot?'🔥':''}</h3><p>${escapeHtml(a.profile||'暂无账号画像')}</p><div class="moli-weibo-profile-actions"><button data-profile-edit>编辑资料</button><button data-profile-dm>发私信</button><button data-profile-wechat>+微信好友</button></div><div class="moli-weibo-spark-callout"><span>添加小火花，成为持续网友吧！</span><button data-profile-hot>${a.hot?'🔥':'🔥'}</button></div></div>`,'账号资料',{root:false});overlay.querySelector('[data-profile-edit]').onclick=()=>{const next=windowRef.prompt?.(`编辑 @${a.name} 的账号设定`,a.profile||'');if(next!==null){saveWeiboFollow(scopeKey,{...a,profile:String(next||'').trim()});renderProfile(id);}};overlay.querySelector('[data-profile-dm]').onclick=()=>renderChat(id);overlay.querySelector('[data-profile-wechat]').onclick=()=>{const existing=getContacts().find(c=>String(c.networkAccountId||'')===String(a.id));if(existing){toast(`${displayName(existing)} 已经是微信好友`);return;}const created=createCustomContact({name:a.name,customAvatar:a.avatar||'',intro:a.profile||'',prompt:[a.profile||'',Array.isArray(a.memory)&&a.memory.length?`【此前网络经历】\n${a.memory.slice(-20).join('\n')}`:''].filter(Boolean).join('\n\n'),customRoleMode:'global',contactOrigin:'network',networkAccountId:a.id,networkProfile:a.profile||''});createPrivateConversationInstance(scopeKey,created.id,{scopeMode:'global'});saveWeiboFollow(scopeKey,{...a,contactId:created.id});saveNetworkActor(scopeKey,{publicId:a.id,name:a.name,profile:a.profile||'',avatar:a.avatar||'',contactId:created.id,hot:Boolean(a.hot),memory:Array.isArray(a.memory)?a.memory:[]});linkNetworkActorContact(scopeKey,a.id,created.id);toast(`${a.name} 已添加为微信好友`);renderProfile(id);};overlay.querySelector('[data-profile-hot]').onclick=()=>{setWeiboFollowHot(scopeKey,id,!a.hot);renderProfile(id);};overlay.querySelector('[data-profile-avatar]').onclick=()=>{const input=document.createElement('input');input.type='file';input.accept='image/*';input.onchange=()=>{const file=input.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{saveWeiboFollow(scopeKey,{...a,avatar:String(reader.result||'')});renderProfile(id);};reader.readAsDataURL(file);};input.click();};};
    const renderMessages=()=>{const u=unread(),groups=listWeiboFanGroups(scopeKey);const followed=follows(),known=new Map(followed.map(x=>[String(x.id),x]));for(const peer of listWeiboMessagePeers(scopeKey)){if(!known.has(String(peer.id)))known.set(String(peer.id),peer);}const people=[...known.values()].filter(x=>listWeiboPrivateMessages(scopeKey,x.id).length).sort((a,b)=>Number(Boolean(b.hot))-Number(Boolean(a.hot))||Number((listWeiboPrivateMessages(scopeKey,b.id).at(-1)||{}).createdAt||0)-Number((listWeiboPrivateMessages(scopeKey,a.id).at(-1)||{}).createdAt||0));shell(`<div class="moli-weibo-network-search">⌕ 搜索聊天记录、群号</div><div class="moli-weibo-network-list"><button class="moli-weibo-message-special" data-special="mentions"><span class="at">@</span><b>@我的</b>${u.mentions?`<i>${u.mentions}</i>`:''}</button><button class="moli-weibo-message-special" data-special="comments"><span class="comment">▣</span><b>评论</b>${u.comments?`<i>${u.comments}</i>`:''}</button>${groups.map(g=>`<button class="moli-weibo-message-person" data-group-open="${escapeHtml(g.id)}"><span class="moli-weibo-network-avatar"><span>群</span></span><span><b>${escapeHtml(g.name)}</b><small>${escapeHtml(g.messages?.at(-1)?.content||`${(g.memberIds||[]).length}位成员`)}</small></span></button>`).join('')}${people.map(x=>{const ms=listWeiboPrivateMessages(scopeKey,x.id),last=ms.at(-1),n=ms.filter(m=>m.role==='account'&&Number(m.createdAt||0)>Number(getWeiboMessageReadState(scopeKey)[`dm:${x.id}`]||0)).length;return`<button class="moli-weibo-message-person" data-dm-open="${escapeHtml(x.id)}"><span class="moli-weibo-network-avatar">${avatar(x)}</span><span><b>${escapeHtml(x.name)} ${x.hot?'🔥':''}</b><small>${escapeHtml(last?.content||'')}</small></span>${n?`<i>${n}</i>`:''}</button>`;}).join('')}</div>`,'消息');overlay.querySelectorAll('[data-dm-open]').forEach(b=>b.onclick=()=>renderChat(b.dataset.dmOpen));overlay.querySelectorAll('[data-dm-open]').forEach(b=>{let timer=0;const clear=()=>{if(timer){clearTimeout(timer);timer=0;}};const remove=()=>{if(windowRef.confirm?.('删除与这个账号的全部私信记录？')){clearWeiboPrivateMessages(scopeKey,b.dataset.dmOpen);renderMessages();}};b.addEventListener('pointerdown',()=>{timer=setTimeout(remove,650)});b.addEventListener('pointerup',clear);b.addEventListener('pointerleave',clear);b.addEventListener('contextmenu',e=>{e.preventDefault();remove();});});overlay.querySelectorAll('[data-group-open]').forEach(b=>b.onclick=()=>renderGroup(b.dataset.groupOpen));overlay.querySelectorAll('[data-special]').forEach(b=>b.onclick=()=>renderNotifications(b.dataset.special));};
    const renderNotifications=kind=>{const rows=notifications()[kind]||[];markWeiboMessageRead(scopeKey,kind);shell(`<div class="moli-weibo-network-list">${rows.map(x=>`<button class="moli-weibo-notice-row" data-notice-post="${escapeHtml(x.postId)}"><b>${escapeHtml(x.author)}</b><small>${escapeHtml(x.content)}</small></button>`).join('')||'<p class="moli-web-muted">暂时没有新消息。</p>'}</div>`,kind==='mentions'?'@我的':'评论',{root:false});overlay.querySelectorAll('[data-notice-post]').forEach(b=>b.onclick=()=>{overlay.remove();openedPublicWebPostId=String(b.dataset.noticePost);renderPublicWeb();});};
    const renderChat=id=>{const a=follows().find(x=>String(x.id)===String(id))||listWeiboMessagePeers(scopeKey).find(x=>String(x.id)===String(id));if(!a)return renderMessages();const msgs=listWeiboPrivateMessages(scopeKey,id);markWeiboMessageRead(scopeKey,`dm:${id}`);shell(`<div class="moli-weibo-chat-identity"><span class="moli-weibo-network-avatar">${avatar(a)}</span><b>${escapeHtml(a.name)} ${a.hot?'🔥':''}</b>${a.hot?'':`<div class="moli-weibo-chat-spark-tip"><span>添加小火花，成为持续网友吧！</span><button data-chat-hot>🔥</button></div>`}</div><div class="moli-weibo-chat-list">${msgs.map(m=>`<div class="moli-weibo-chat-bubble ${m.role==='user'?'is-user':'is-account'}" data-dm-message-id="${escapeHtml(m.id)}">${escapeHtml(m.content)}</div>`).join('')||'<p class="moli-web-muted">还没有私信。</p>'}</div><div class="moli-weibo-chat-compose"><button class="moli-weibo-chat-emoji" data-dm-emoji aria-label="表情">☺</button><textarea data-dm-input rows="2" placeholder="发私信…"></textarea><button data-dm-send>＋</button></div>`,'',{root:false});overlay.querySelector('[data-chat-hot]')?.addEventListener('click',()=>{const existing=follows().find(x=>String(x.id)===String(id));if(existing)setWeiboFollowHot(scopeKey,id,!existing.hot);else saveWeiboFollow(scopeKey,{id:a.id,name:a.name,profile:a.profile||'',avatar:a.avatar||'',source:'message',hot:true});renderChat(id);});overlay.querySelectorAll('[data-dm-message-id]').forEach(el=>{let timer=0;const clear=()=>{if(timer){clearTimeout(timer);timer=0;}};const remove=()=>{if(windowRef.confirm?.('删除这条私信？')){deleteWeiboPrivateMessage(scopeKey,id,el.dataset.dmMessageId);renderChat(id);}};el.addEventListener('pointerdown',()=>{timer=setTimeout(remove,650)});el.addEventListener('pointerup',clear);el.addEventListener('pointerleave',clear);el.addEventListener('contextmenu',e=>{e.preventDefault();remove();});});overlay.querySelector('[data-dm-emoji]').onclick=()=>{windowRef.alert?.('表情包入口已预留。');};overlay.querySelector('[data-dm-send]').onclick=async()=>{const ta=overlay.querySelector('[data-dm-input]'),text=String(ta?.value||'').trim();if(text){addWeiboPrivateMessage(scopeKey,id,{role:'user',content:text});renderChat(id);return;}const btn=overlay.querySelector('[data-dm-send]');btn.disabled=true;try{const latest=follows().find(x=>String(x.id)===String(id))||a;const reply=await generateWeiboPrivateReply({scopeKey,account:latest,messages:listWeiboPrivateMessages(scopeKey,id)});if(reply)addWeiboPrivateMessage(scopeKey,id,{role:'account',content:reply});}catch(error){windowRef.alert?.(`私信回复失败：${error?.message||error}`);}renderChat(id);};};
    const openGroupCreate=()=>{const rows=follows();shell(`<div class="moli-weibo-group-create"><label>群名<input data-group-name placeholder="给粉丝群起个名字"></label><p>选择关注人</p>${rows.map(x=>`<label class="moli-weibo-group-member"><input type="checkbox" value="${escapeHtml(x.id)}"><span class="moli-weibo-network-avatar">${avatar(x)}</span><b>${escapeHtml(x.name)} ${x.hot?'🔥':''}</b></label>`).join('')}<button class="primary" data-group-create>新建粉丝群</button></div>`,'新建粉丝群',{root:false});overlay.querySelector('[data-group-create]').onclick=()=>{const ids=[...overlay.querySelectorAll('.moli-weibo-group-member input:checked')].map(x=>x.value);const name=String(overlay.querySelector('[data-group-name]')?.value||'').trim();if(!ids.length){windowRef.alert?.('至少选择一个关注人。');return;}const g=saveWeiboFanGroup(scopeKey,{name:name||'粉丝群',memberIds:ids});renderGroup(g.id);};};
    const renderGroup=id=>{const g=listWeiboFanGroups(scopeKey).find(x=>String(x.id)===String(id));if(!g)return renderMessages();const members=follows().filter(x=>(g.memberIds||[]).includes(String(x.id)));shell(`<div class="moli-weibo-group-members">${members.map(x=>`<span>${avatar(x)}${escapeHtml(x.name)}</span>`).join('')}</div><div class="moli-weibo-chat-list">${(g.messages||[]).map(m=>`<div class="moli-weibo-chat-bubble ${m.role==='user'?'is-user':'is-account'}" data-dm-message-id="${escapeHtml(m.id)}">${escapeHtml(m.content)}</div>`).join('')||'<p class="moli-web-muted">粉丝群已创建，可以开始聊天。</p>'}</div><div class="moli-weibo-chat-compose"><textarea data-group-input rows="2" placeholder="发消息…"></textarea><button data-group-send>＋</button></div>`,g.name,{root:false});overlay.querySelector('[data-group-send]').onclick=()=>{const ta=overlay.querySelector('[data-group-input]'),text=String(ta?.value||'').trim();if(!text)return;addWeiboFanGroupMessage(scopeKey,id,{role:'user',senderName:userName(),content:text});renderGroup(id);};};
    panel.appendChild(overlay);overlay.addEventListener('click',e=>{if(e.target===overlay)overlay.remove();});renderRoot();
  };
  const openWeiboComposeDialog=()=>{
    const overlay=document.createElement('div'); overlay.className='moli-weibo-compose-overlay';
    overlay.innerHTML=`<div class="moli-weibo-compose-page"><div class="moli-weibo-compose-head"><button data-weibo-dialog-close>取消</button><div><b>发微博</b><button type="button" class="moli-weibo-compose-account" data-weibo-compose-account>大号 · ${escapeHtml(getTavernUserContext()?.name||'User')}⌄</button></div><button class="send" data-action="weibo-publish">发送</button></div><div class="moli-weibo-compose-body"><textarea data-weibo-compose-text placeholder="分享新鲜事…"></textarea><div class="moli-weibo-compose-at-hint" data-weibo-compose-at-hint hidden>可写角色名字（强制角色用大号回应）、角色的小号、已出现过的路人ID</div><div class="moli-weibo-compose-images" data-weibo-compose-images hidden></div></div><div class="moli-weibo-compose-tools"><button data-action="weibo-media-add" aria-label="图片/视频">▧</button><button data-action="weibo-insert-at" aria-label="提及">@</button><button data-action="weibo-insert-topic" aria-label="话题">#</button></div></div>`;
    panel.appendChild(overlay); const close=()=>overlay.remove(); overlay.querySelector('[data-weibo-dialog-close]').onclick=close;
    const body=overlay.querySelector('[data-weibo-compose-text]'); const hint=overlay.querySelector('[data-weibo-compose-at-hint]'); let atHintShown=false; const media=[]; const profile=getCommunityUserProfile(getScopeKey?.()); let composeIdentity={mode:'real',name:getTavernUserContext()?.name||'User',id:'user'}; const accountButton=overlay.querySelector('[data-weibo-compose-account]'); const chooseAccount=()=>{overlay.querySelector('[data-weibo-account-menu]')?.remove();const menu=document.createElement('div');menu.className='moli-weibo-account-menu';const smallIds=(profile.communityIds||[]).filter(x=>x.type==='小号');menu.setAttribute('data-weibo-account-menu','1');menu.innerHTML=`<button data-account-mode="real">大号 · ${escapeHtml(getTavernUserContext()?.name||'User')}</button>${smallIds.map(x=>`<button data-account-mode="small" data-account-id="${escapeHtml(x.id)}" data-account-name="${escapeHtml(x.name)}">小号 · ${escapeHtml(x.name)}</button>`).join('')||'<small>暂无小号，请先在自创板块 ID card 保存。</small>'}`;accountButton.after(menu);menu.onclick=e=>{const b=e.target.closest('button[data-account-mode]');if(!b)return;if(b.dataset.accountMode==='small')composeIdentity={mode:'small',name:String(b.dataset.accountName||'小号用户'),id:String(b.dataset.accountId||'')};else composeIdentity={mode:'real',name:getTavernUserContext()?.name||'User',id:'user'};accountButton.textContent=`${composeIdentity.mode==='small'?'小号':'大号'} · ${composeIdentity.name}⌄`;menu.remove();};}; accountButton.onclick=chooseAccount;
    const refreshMedia=()=>{const box=overlay.querySelector('[data-weibo-compose-images]');box.hidden=!media.length;box.innerHTML=media.map((m,i)=>`<div class="moli-weibo-compose-media-tile${m.type==='video'?' is-video':''}"><button data-action="weibo-media-remove" data-media-index="${i}">×</button><small>${m.type==='video'?'视频':'图片'}</small><span>${escapeHtml(m.text)}</span></div>`).join('')+(media.length<9&&media.every(m=>m.type!=='video')?`<button class="moli-weibo-compose-media-plus" data-action="weibo-media-add" aria-label="继续添加">＋</button>`:'');};
    const openMediaEditor=()=>{const dialog=document.createElement('div');dialog.className='moli-weibo-media-editor';dialog.innerHTML=`<div class="moli-weibo-media-editor-card"><div class="moli-weibo-media-editor-tabs"><button class="is-active" data-media-type="image">图片</button><button data-media-type="video">视频</button></div><textarea data-media-text rows="5" placeholder="图片（最多九张）：填写内容"></textarea><div class="moli-weibo-media-editor-actions"><button data-media-cancel>取消</button><button class="primary" data-media-confirm>添加</button></div></div>`;overlay.appendChild(dialog);let type='image';const ta=dialog.querySelector('[data-media-text]');dialog.querySelectorAll('[data-media-type]').forEach(b=>b.onclick=()=>{type=b.dataset.mediaType;dialog.querySelectorAll('[data-media-type]').forEach(x=>x.classList.toggle('is-active',x===b));ta.placeholder=type==='video'?'视频（最多一个）：填写内容':'图片（最多九张）：填写内容';});dialog.querySelector('[data-media-cancel]').onclick=()=>dialog.remove();dialog.querySelector('[data-media-confirm]').onclick=()=>{const text=String(ta.value||'').trim();if(!text)return;if(type==='video'){const old=media.findIndex(x=>x.type==='video');if(old>=0)media.splice(old,1);media.push({type,text});}else if(media.filter(x=>x.type==='image').length<9){media.push({type,text});}dialog.remove();refreshMedia();};ta.focus();};
    body.addEventListener('input',()=>{if(atHintShown)hint.hidden=true;});hint.addEventListener('click',()=>{hint.hidden=true;body.focus();});
    overlay.addEventListener('click',e=>{const add=e.target.closest?.('[data-action="weibo-media-add"]');if(add){openMediaEditor();return;}const rm=e.target.closest?.('[data-action="weibo-media-remove"]');if(rm){media.splice(Number(rm.dataset.mediaIndex),1);refreshMedia();return;}});
    overlay.querySelector('[data-action="weibo-insert-at"]').onclick=()=>{body.setRangeText('@',body.selectionStart,body.selectionEnd,'end');if(!atHintShown){hint.hidden=false;atHintShown=true;}body.focus();};
    overlay.querySelector('[data-action="weibo-insert-topic"]').onclick=()=>{body.setRangeText('##',body.selectionStart,body.selectionEnd,'end');body.selectionStart=body.selectionEnd=Math.max(0,body.selectionStart-1);body.focus();};
    overlay.querySelector('[data-action="weibo-publish"]').onclick=()=>{const content=String(body.value||'').trim();const images=media.filter(x=>x.type==='image').map(x=>x.text).slice(0,9);const videos=media.filter(x=>x.type==='video').map(x=>x.text).slice(0,1);if(!content&&!images.length&&!videos.length){windowRef.alert?.('写点内容或添加图片/视频后再发送。');return;}const userName=getTavernUserContext()?.name||'User';const tags=[...content.matchAll(/#[^#\s]{1,40}#/g)].map(m=>m[0]).slice(0,12);const author=composeIdentity.mode==='small'?{type:'internet_actor',id:composeIdentity.id||'',name:composeIdentity.name,anonymous:true,ownerKind:'local-user'}:{type:'user',id:'user',name:userName,anonymous:false};createPublicWebPost(getScopeKey?.(),{section:'weibo',type:'weibo',author,title:content.slice(0,40),content,tags,extra:{weiboLane:'实时',images,videos,userOwned:true,userIdentityMode:composeIdentity.mode,userIdentityId:composeIdentity.id||'',reposts:0,hotScore:0}});close();renderPublicWeb();}; body.focus();
  };
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event => {
    const weiboMessages=event.target?.closest?.('[data-action="weibo-messages"]');if(weiboMessages){openWeiboMessagesDialog();return;}
    const weiboSearch=event.target?.closest?.('[data-action="weibo-search"]');if(weiboSearch){openWeiboSearchDialog();return;}
    const weiboCompose=event.target?.closest?.('[data-action="weibo-compose"]');if(weiboCompose){openWeiboComposeDialog();return;}
    ...
    const fixedPersonasToggle=event.target?.closest?.('[data-action="toggle-fixed-personas-community"]'); if(fixedPersonasToggle){const current=getPublicWebSettings(getScopeKey?.()).fixedPersonasCommunityEnabled;updatePublicWebSettings(getScopeKey?.(),{fixedPersonasCommunityEnabled:!current});renderPublicWeb();return;}
    ...
    if(xhsRefresh){if(publicWebGenerating.has('xiaohongshu'))return;publicWebGenerating.add('xiaohongshu');xhsRefresh.disabled=true;renderPublicWeb();try{const items=await generatePublicWebRefresh({scopeKey:getScopeKey?.(),fixedPersonasCommunityEnabled:getPublicWebSettings(getScopeKey?.()).fixedPersonasCommunityEnabled,section:'xiaohongshu'});settleRefreshedCharacterPosts(items);const createdPosts=replacePublicWebSectionPosts(getScopeKey?.(),'xiaohongshu',items);await settleCommunityDiscovery(createdPosts,getPublicWebSettings(getScopeKey?.()));openedPublicWebPostId='';renderPublicWeb();}catch(error){console.error('[moli小手机] xhs refresh failed:',error);windowRef.alert?.(`刷新失败：${error?.message||error}`);}finally{publicWebGenerating.delete('xiaohongshu');renderPublicWeb();}return;}
    ...
    const weiboRefresh=event.target?.closest?.('[data-action="public-web-refresh"]'); if(weiboRefresh&&currentPublicWebTab==='weibo'){if(publicWebGenerating.has('weibo'))return;publicWebGenerating.add('weibo');renderPublicWeb();try{const items=await generatePublicWebRefresh({scopeKey:getScopeKey?.(),fixedPersonasCommunityEnabled:getPublicWebSettings(getScopeKey?.()).fixedPersonasCommunityEnabled,section:'weibo'});settleRefreshedCharacterPosts(items);const createdPosts=appendPublicWebSectionPosts(getScopeKey?.(),'weibo',items);await settleCommunityDiscovery(createdPosts,getPublicWebSettings(getScopeKey?.()));if(Array.isArray(items.hotTopics))setWeiboHotTopics(getScopeKey?.(),items.hotTopics);for(const batch of (Array.isArray(items.userPostComments)?items.userPostComments:[]))settleWeiboUserPostEcology(getScopeKey?.(),batch?.postId,batch?.comments);trimWeiboLanePosts(getScopeKey?.(),15);for(const item of items){const message=String(item?.extra?.privateMessage||'').trim();const id=String(item?.author?.id||item?.author?.name||'').trim();const name=String(item?.author?.name||id).trim();if(message&&id&&name){saveWeiboMessagePeer(getScopeKey?.(),{id,name,profile:'由微博公开互动产生的持续网络账号'});addWeiboPrivateMessage(getScopeKey?.(),id,{role:'account',content:message});}}openedPublicWebPostId='';renderPublicWeb();}catch(error){console.error('[moli小手机] weibo refresh failed:',error);windowRef.alert?.(`刷新失败：${error?.message||error}`);}finally{publicWebGenerating.delete('weibo');renderPublicWeb();}return;}
    ...
    const weiboToggleReplies=event.target?.closest?.('[data-action="weibo-toggle-replies"]');if(weiboToggleReplies){const id=String(weiboToggleReplies.dataset.commentId||'');if(expandedWeiboThreads.has(id))expandedWeiboThreads.delete(id);else expandedWeiboThreads.add(id);renderPublicWeb();return;}
    const weiboCommentsRefresh=event.target?.closest?.('[data-action="weibo-comments-refresh"]');if(weiboCommentsRefresh){const postId=String(weiboCommentsRefresh.dataset.postId||'');const busyKey=`weibo-comments:${postId}`;if(publicWebGenerating.has(busyKey))return;publicWebGenerating.add(busyKey);renderPublicWeb();try{const post=getPublicWebPost(getScopeKey?.(),postId);await settleQueuedCommunityTargets(post);await settleNaturalCommunityParticipants(post);const additions=await generateWeiboCommentRefresh({scopeKey:getScopeKey?.(),post});for(const item of additions)addPublicWebComment(getScopeKey?.(),postId,item);consumeUserCommentPending(postId,null);}catch(error){windowRef.alert?.(`刷新微博评论失败：${error?.message||error}`);}finally{publicWebGenerating.delete(busyKey);renderPublicWeb();}return;}
    const weiboCommentReply=event.target?.closest?.('[data-action="weibo-comment-reply"]');if(weiboCommentReply){openCommunityComposer({postId:weiboCommentReply.dataset.postId,replyToCommentId:weiboCommentReply.dataset.commentId,replyLabel:`回复 @${weiboCommentReply.dataset.commentAuthor||'网友'}`});return;}
    const weiboCommentRepost=event.target?.closest?.('[data-action="weibo-comment-repost"]');if(weiboCommentRepost){const post=getPublicWebPost(getScopeKey?.(),weiboCommentRepost.dataset.postId);const c=(post?.comments||[]).find(x=>String(x.id)===String(weiboCommentRepost.dataset.commentId));if(c){const text=windowRef.prompt?.('转发这条评论（可补充一句）','')??null;if(text!==null){createPublicWebPost(getScopeKey?.(),{section:'weibo',type:'weibo',author:{type:'user',id:'user',name:getTavernUserContext()?.name||'User'},title:'转发评论',content:String(text||'转发评论'),extra:{weiboLane:'实时',repostText:`@${communityAuthorDisplay(c.author)}：${c.content}`,reposts:0,userOwned:true}});renderPublicWeb();}}return;}
    const weiboLike=event.target?.closest?.('[data-action="weibo-like"]');if(weiboLike){togglePublicWebLike(getScopeKey?.(),weiboLike.dataset.postId,'user');renderPublicWeb();return;}
    const weiboRepost=event.target?.closest?.('[data-action="weibo-repost"]');if(weiboRepost){const original=getPublicWebPost(getScopeKey?.(),weiboRepost.dataset.postId);if(!original)return;const text=windowRef.prompt?.('转发微博（可以留空直接转发）','')??null;if(text===null)return;const userName=getTavernUserContext()?.name||'User';const originalName=communityAuthorDisplay(original.author);createPublicWebPost(getScopeKey?.(),{section:'weibo',type:'weibo',author:{type:'user',id:'user',name:userName},title:String(text||original.title||'转发微博').slice(0,40),content:String(text||'转发微博'),extra:{weiboLane:'关注',repostText:`@${originalName}：${String(original.content||original.title||'').slice(0,240)}`,repostAuthor:{id:String(original.author?.id||originalName),name:originalName},repostChain:[...(original.extra?.repostChain||[])].slice(0,6),reposts:0,hotScore:0,userOwned:true}});incrementWeiboRepost(getScopeKey?.(),original.id);openedPublicWebPostId='';renderPublicWeb();return;}
    const share=event.target?.closest?.('[data-action="public-web-share"]'); if(share){
      ...
      const forwardSnapshot={postId:String(post.id||''),section:String(post.section||''),platform,customCommunityId:String(post.extra?.customCommunityId||''),customCommunityName:String(post.extra?.customCommunityName||''),authorName:String(post.author?.name||post.authorName||'小号网友'),title:String(post.title||'无标题'),snapshotAt:Date.now()};
  ...
  communityComposer.innerHTML=`<div class="moli-community-composer-shell"><div class="moli-community-composer-top"><button type="button" class="moli-community-pill" data-community-identity-button>大号</button><div class="moli-community-at-wrap"><button type="button" class="moli-community-pill moli-community-at-pill" data-community-at-button>@</button><div class="moli-community-at-hint" data-community-at-hint hidden>可写角色名字（强制角色用大号回应）、角色的小号、已出现过的路人ID</div></div></div><div class="moli-community-picker" data-community-identity-menu hidden><button data-community-mode="real">大号</button><button data-community-mode="anonymous">小号 ›</button><button data-community-mode="owner">楼主</button><div class="moli-community-id-side" data-community-id-side hidden></div></div><div class="moli-community-composer-panel"><div class="moli-community-alias-row" data-community-alias-row hidden>小号：<input data-community-alias placeholder="小号名"></div><span data-community-reply-target hidden></span><div class="moli-community-textarea-wrap"><textarea data-community-content placeholder="说点什么……"></textarea></div><button type="button" class="moli-community-send" data-community-send>发送</button></div></div>`;
  ...
  communityComposer.querySelector('[data-community-identity-menu]')?.addEventListener('click',e=>{const idPick=e.target.closest('[data-community-id-name]');if(idPick){const name=String(idPick.dataset.communityIdName||'');communityComposerState.identityMode='anonymous';communityComposer.querySelector('[data-community-identity-button]').textContent='小号';const alias=communityComposer.querySelector('[data-community-alias]');if(alias)alias.value=name;const row=communityComposer.querySelector('[data-community-alias-row]');if(row){row.hidden=false;row.childNodes[0].textContent='ID：';}e.currentTarget.hidden=true;return;}const b=e.target.closest('[data-community-mode]');if(!b)return;const mode=b.dataset.communityMode;if(mode==='anonymous'){const side=e.currentTarget.querySelector('[data-community-id-side]');const ids=getCommunityUserProfile(getScopeKey?.()).communityIds.filter(x=>x.type==='小号');side.innerHTML=ids.map(x=>`<button data-community-id-name="${escapeHtml(x.name)}">${escapeHtml(x.name)}</button>`).join('')||'<small>请先在 ID card 添加小号</small>';side.hidden=false;return;}communityComposerState.identityMode=mode;communityComposer.querySelector('[data-community-identity-button]').textContent=({real:'大号',owner:'楼主'})[mode];communityComposer.querySelector('[data-community-alias-row]').hidden=true;e.currentTarget.hidden=true;});
  communityComposer.querySelector('[data-community-at-button]')?.addEventListener('click',()=>{
    const input=communityComposer.querySelector('[data-community-content]'); if(!input)return;
    const pos=Number.isInteger(input.selectionStart)?input.selectionStart:input.value.length; const before=input.value.slice(0,pos),after=input.value.slice(pos); const prefix=before&&!/\s$/.test(before)?' ':'';
    input.value=`${before}${prefix}@${after}`; const caret=(before+prefix+'@').length; input.focus(); input.setSelectionRange?.(caret,caret); const hint=communityComposer.querySelector('[data-community-at-hint]');if(hint)hint.hidden=false;
  });
  communityComposer.querySelector('[data-community-at-hint]')?.addEventListener('click',e=>{e.currentTarget.hidden=true;communityComposer.querySelector('[data-community-content]')?.focus();});
  communityComposer.querySelector('[data-community-content]')?.addEventListener('input',e=>{const hint=communityComposer.querySelector('[data-community-at-hint]');if(hint&&String(e.target.value||'').trim()!=='@')hint.hidden=true;});
  ...
  const communityPostKnowledgeText = (post, snapshotAt = Date.now()) => {
    ...
    const actor = author => String(author?.name || '小号网友').trim() || '小号网友';
  ...
  const communitySurfaceActors = post => {
    const rows=[];const seen=new Set();const add=author=>{if(!author?.name)return;const name=String(author.name||'').trim();if(!name)return;const key=`${String(author.type||'')}:${String(author.id||'')}:${name}`;if(seen.has(key))return;seen.add(key);rows.push({name,author});};const walk=items=>{for(const item of items||[]){add(item?.author);walk(item?.replies||[]);}};add(post?.author);walk(post?.comments||[]);for(const answer of post?.extra?.answers||[]){add(answer?.author);walk(answer?.comments||[]);}return rows;
  };
  const resolveCommunityMentions=(post,content)=>{
    const text=String(content||''),out=[],used=new Set(),contacts=communityCallableContacts(),candidates=[];
    for(const contact of contacts){const name=String(displayName(contact)||'').trim();if(name)candidates.push({name,kind:'contact',contact,mode:'real'});}
    for(const row of communitySurfaceActors(post)){const author=row.author,name=row.name;if(String(author?.type||'')==='contact'&&author?.id){const contact=contacts.find(x=>String(x.id)===String(author.id));if(contact)candidates.push({name,kind:'contact',contact,mode:String(displayName(contact))===name?'real':'surface',alias:name});}else if(String(author?.type||'')==='internet_actor'&&!author?.ownerKind)candidates.push({name,kind:'passerby',author});}
    candidates.sort((x,y)=>y.name.length-x.name.length);for(const c of candidates){if(!text.includes(`@${c.name}`))continue;const key=c.kind==='contact'?`contact:${c.contact.id}:${c.mode}:${c.alias||''}`:`passerby:${c.author.id||c.name}`;if(used.has(key))continue;used.add(key);out.push(c);}return out;
  };
  const passerbyThreadContext=(post,actor)=>{const name=String(actor?.name||''),id=String(actor?.id||''),rows=[];const same=a=>a&&((id&&String(a.id||'')===id)||String(a.name||'')===name);const walk=(items,label='评论')=>{for(const item of items||[]){if(same(item?.author))rows.push(`${label}：${String(item.content||'').trim()}`);walk(item?.replies||[],'回复');}};if(same(post?.author))rows.push(`作为发帖者：${String(post.content||'').trim()}`);walk(post?.comments||[]);for(const answer of post?.extra?.answers||[]){if(same(answer?.author))rows.push(`回答：${String(answer.content||'').trim()}`);walk(answer?.comments||[],'回答下评论');}return rows.slice(-20).join('\n');};
  ...
  communityComposer.querySelector('[data-community-send]')?.addEventListener('click',()=>{const state=communityComposerState;if(!state)return;const post=getPublicWebPost(getScopeKey?.(),state.postId);if(!post)return;const content=String(communityComposer.querySelector('[data-community-content]')?.value||'').trim();if(!content)return;const mode=state.identityMode||'real';let author;if(mode==='anonymous'){const alias=String(communityComposer.querySelector('[data-community-alias]')?.value||'').trim()||'小号用户';setAnonymousAlias(state.postId,alias);author={type:'internet_actor',id:'',name:alias,anonymous:true,ownerKind:'local-user'};}else if(mode==='owner'){author={type:'user',id:'user',name:'楼主',uiName:'楼主',anonymous:false,knownIdentityId:'user'};}else author={type:'user',id:'user',name:getTavernUserContext()?.name||'User',anonymous:false};const entry={id:`userc_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,author,content,replyToCommentId:state.replyToCommentId};let saved;if(post.section==='zhihu'&&state.composeMode==='zhihu-answer'){saved={id:`zua_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,author,content,comments:[],upvotes:0};addZhihuAnswer(getScopeKey?.(),post.id,saved);}else if(post.section==='zhihu'&&state.zhihuAnswerId){saved={...entry};addZhihuAnswerComments(getScopeKey?.(),post.id,state.zhihuAnswerId,[saved]);}else saved=addPublicWebComment(getScopeKey?.(),post.id,entry);if(saved){recordCommunityUserFact({post,saved,state,content});if(post.section==='weibo'){const ids=new Set();const addActor=a=>{const id=String(a?.id||a?.name||'').trim();if(id&&id!=='user')ids.add(id);};addActor(post.author);if(state.replyToCommentId){const target=(post.comments||[]).find(x=>String(x.id)===String(state.replyToCommentId));addActor(target?.author);}for(const id of ids)rememberWeiboPublicInteraction(getScopeKey?.(),id,`User 在微博中对你相关的公开讨论说：“${content.slice(0,500)}”`);}}const mentions=resolveCommunityMentions(post,content);const answerId=state.zhihuAnswerId;if(saved&&state.composeMode!=='zhihu-answer')queueCommunityPending({type:'user-comment',postId:String(post.id),answerId:String(answerId||''),commentId:String(saved.id||''),content:String(saved.content||'')});for(const mention of mentions){if(mention.kind==='contact'){const queuedMention=queueCommunityPending({type:'mention',kind:'comment',postId:String(post.id),answerId:String(answerId||''),replyToCommentId:String(saved?.id||''),commentId:String(saved?.id||''),content:String(saved?.content||''),targetId:String(mention.contact.id),mentionMode:mention.mode,mentionAlias:String(mention.alias||'')});recordWorldEvent(getScopeKey?.(),{source:`community.${post.section||'unknown'}`,actorId:'user',action:'MENTION',targetContactIds:[mention.contact.id],objectId:String(queuedMention.id),content:`User 在${sourceLabel(post)}中 @了你：${String(saved?.content||'').trim()}`,metadata:{postId:String(post.id||''),commentId:String(saved?.id||''),pendingRefresh:true,mentionMode:mention.mode,mentionAlias:String(mention.alias||'')},awareness:'pending',dedupeKey:`community-mention:${queuedMention.id}`});}else if(mention.kind==='passerby'){queueCommunityPending({type:'passerby-mention',postId:String(post.id),answerId:String(answerId||''),replyToCommentId:String(saved?.id||''),commentId:String(saved?.id||''),content:String(saved?.content||''),actor:mention.author});}}closeCommunityComposer();renderPublicWeb();});
  ...
  const renderStorageAudit = async () => {
    ...
    try {
      ...
      const describeStorageKey = key => {
        ...
        if (value.startsWith('moli-phone:identity-awareness:')) return '小号身份认知';
