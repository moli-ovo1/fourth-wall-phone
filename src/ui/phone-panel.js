...
你只生成你自己这一名角色的一条公开${isAnswer?'回答':'回复'}；不要代替其他受邀角色，也不要顺手生成路人/网友评论。严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择匿名时使用的匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 1~3 个 <msg>私聊内容</msg>。`;
      try{
        ...
        let publicDid=false;let writtenPublicId='';if(isAnswer){const text=String(raw.match(/<community_answer>([\s\S]*?)<\/community_answer>/i)?.[1]||'').trim();if(text){const written=addZhihuAnswer(scopeKey,post.id,{author,content:text});writtenPublicId=String(written?.id||'');publicDid=Boolean(writtenPublicId);}}else{const text=String(raw.match(/<community_reply>([\s\S]*?)<\/community_reply>/i)?.[1]||'').trim();if(text){if(post.section==='zhihu'&&item.answerId){const before=(getPublicWebPost(scopeKey,post.id)?.extra?.answers||[]).find(a=>String(a.id)===String(item.answerId))?.comments?.length||0;addZhihuAnswerComments(scopeKey,post.id,item.answerId,[{author,content:text,replyToCommentId:String(item.replyToCommentId||'')}]);const fresh=(getPublicWebPost(scopeKey,post.id)?.extra?.answers||[]).find(a=>String(a.id)===String(item.answerId));const added=(fresh?.comments||[]).slice(before).find(c=>String(c?.author?.id||c?.author?.knownIdentityId||'')===String(target.id)&&String(c?.content||'').trim()===text);writtenPublicId=String(added?.id||'');publicDid=Boolean(writtenPublicId);}else{const written=addPublicWebComment(scopeKey,post.id,{author,content:text,replyToCommentId:String(item.replyToCommentId||'')});writtenPublicId=String(written?.id||'');publicDid=Boolean(writtenPublicId);}}}
        if(!publicDid) throw new Error(`角色必须${isAnswer?'回答':'回应'}，但公开内容没有成功写入帖子；本角色不会被标记为已回复，请重试刷新。`);
