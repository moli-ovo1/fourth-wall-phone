export function createPhonePanel({
    ...
    function positionNear(handleElement) {
        ...
        if (top + height > windowRef.innerHeight - 8) top = rect.top - height - 9;
    ...
    panel.addEventListener('pointerdown', event => {
        ...
        if (!nav || event.target.closest?.('button') || !panel.classList.contains('open')) return;
        ...
        try { nav.setPointerCapture(event.pointerId); } catch {}
    ...
    function endPanelDrag(event) {
        if (!panelDragging || (event && event.pointerId !== panelPointerId)) return;
        panelDragging = false;
        panelPointerId = null;
        panel.classList.remove('panel-dragging');
        if (panelMoved) onUiStateChange?.();
    }

    panel.addEventListener('pointerup', endPanelDrag);
    panel.addEventListener('pointercancel', endPanelDrag);

    panel.querySelector('[data-action="settings"]').addEventListener('click', () => showPage('settings'));
    panel.querySelector('[data-action="home"]').addEventListener('click', () => showPage('home'));
    panel.querySelector('[data-action="add"]').addEventListener('click', () => toast('＋ 功能将在后续阶段接入'));
