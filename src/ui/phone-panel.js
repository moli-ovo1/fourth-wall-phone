...
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择小号时使用的小号网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
  ...
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event => {
    ...
    const weiboRefresh=event.target?.closest?.('[data-action="public-web-refresh"]'); if(weiboRefresh&&currentPublicWebTab==='weibo'){if(publicWebGenerating.has('weibo'))return;publicWebGenerating.add('weibo');renderPublicWeb();try{const items=await generatePublicWebRefresh({scopeKey:getScopeKey?.(),fixedPersonasCommunityEnabled:getPublicWebSettings(getScopeKey?.()).fixedPersonasCommunityEnabled,section:'weibo'});appendPublicWebSectionPosts(getScopeKey?.(),'weibo',items);if(Array.isArray(items.hotTopics))setWeiboHotTopics(getScopeKey?.(),items.hotTopics);for(const batch of (Array.isArray(items.userPostComments)?items.userPostComments:[]))settleWeiboUserPostEcology(getScopeKey?.(),batch?.postId,batch?.comments);trimWeiboLanePosts(getScopeKey?.(),15);for(const item of items){const message=String(item?.extra?.privateMessage||'').trim();const id=String(item?.author?.id||item?.author?.name||'').trim();const name=String(item?.author?.name||id).trim();if(message&&id&&name){saveWeiboMessagePeer(getScopeKey?.(),{id,name,profile:'由微博公开互动产生的持续网络账号'});addWeiboPrivateMessage(getScopeKey?.(),id,{role:'account',content:message});}}openedPublicWebPostId='';renderPublicWeb();}catch(error){console.error('[moli小手机] weibo refresh failed:',error);windowRef.alert?.(`刷新失败：${error?.message||error}`);}finally{publicWebGenerating.delete('weibo');renderPublicWeb();}return;}
