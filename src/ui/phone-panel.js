...
export function createPhonePanel({
  ...
  const settleQueuedCommunityTargets = async post => {
    ...
    for(const item of targeted){
      ...
      const instruction=`【moli社区刷新结算｜${isAnswer?'邀请回答':'明确互动'}】\n平台：${sourceLabel(post)}\n标题/问题：${post.title||'无标题'}\n正文/补充：${post.content||'无'}\n当前讨论：\n${communityDiscussionContext(post)}\n\n${item.type==='invite'?`User 明确邀请你${isAnswer?'回答这个问题':'参与跟帖'}`:`User 在评论中明确 @了你：${item.content||''}`}。请按人物自身兴趣、认知和关系真实判断是否公开回应，不要为了展示功能强行回应。公开选择 REPLY 或 SKIP；SKIP 必须给一句简短原因。${proactiveEnabled?'主动私聊权限已开启：公开判断之后，再独立判断是否需要私聊 User；SEND 时给 1~3 条真实手机气泡，SKIP 时给简短原因。':'主动私聊权限关闭：PRIVATE 必须 SKIP。'}\n严格追加：<community_decision>REPLY|SKIP</community_decision><community_reason>公开不回应时的原因</community_reason><community_identity>REAL|ANONYMOUS</community_identity><community_alias>匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 1~3 个 <msg>私聊内容</msg>。`;
      try{
        ...
        markWorldEventsConsumedByObjectTargets(scopeKey,[item.id],'community-settlement'); done.push(item.id); notices.push(`${displayName(target)}${publicDid?(isAnswer?'已回答':'已回应'):`未${isAnswer?'回答':'回应'}：${reason}`}；${privateDid?'已私聊':`未私聊：${privateReason}`}`);
