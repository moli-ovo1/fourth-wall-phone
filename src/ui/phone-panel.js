...
严格追加：<community_decision>REPLY_REAL|REPLY_ANONYMOUS</community_decision><community_alias>选择匿名时使用的匿名网名</community_alias>${isAnswer?'<community_answer>回答正文</community_answer>':'<community_reply>回复正文</community_reply>'}<community_private>SEND|SKIP</community_private><community_private_reason>不私聊时的原因</community_private_reason>；SEND 时再输出 2~5 个 <msg>私聊内容</msg>。`;
  ...
  const openWeiboMessagesDialog=()=>{
    ...
    const shell=(body,title='消息')=>{const u=unread();overlay.innerHTML=`<div class="moli-weibo-network-page"><div class="moli-weibo-network-head"><button data-net-close>‹</button><b>${escapeHtml(title)}</b><button data-net-add aria-label="新建粉丝群">⊕</button></div>${body}<div class="moli-weibo-network-tabs"><button data-net-tab="follows" class="${tab==='follows'?'is-active':''}">关注人</button><button data-net-tab="messages" class="${tab==='messages'?'is-active':''}">消息${u.mentions+u.comments+u.dm?`<i>${u.mentions+u.comments+u.dm}</i>`:''}</button></div></div>`;overlay.querySelector('[data-net-close]').onclick=()=>overlay.remove();overlay.querySelector('[data-net-add]').onclick=openGroupCreate;overlay.querySelectorAll('[data-net-tab]').forEach(b=>b.onclick=()=>{tab=b.dataset.netTab;renderRoot();});};
