...
import { listPublicWebPosts, createPublicWebPost, addPublicWebPosts, getPublicWebPost, addPublicWebComment, togglePublicWebLike, deletePublicWebPost, getPublicWebSettings, updatePublicWebSettings, togglePublicWebFavorite, togglePublicWebPinned, replacePublicWebSectionPosts } from '../storage/public-web-store.js';
...
export function createPhonePanel({
      ...
      <div class="moli-retro-browser">
        ...
        <main class="moli-public-web moli-tianya-browser-body">
          ...
          <div class="moli-tianya-commandbar">
            ...
            <button type="button" data-action="public-web-refresh">[刷新]</button>
          </div>
  ...
  const renderPublicWebDetail = post => {
    ...
    const commonTop = `<div class="moli-web-detail-nav"><button data-action="public-web-detail-back">← 返回</button>${post.section==='tianya'?`<button data-action="tianya-replies-refresh" data-post-id="${escapeHtml(post.id)}">[刷新]</button>`:''}</div>`;
  ...
  panel.querySelector('[data-public-web-feed]')?.addEventListener('click', async event => {
    ...
    if(tianyaFloorReply){const who=tianyaFloorReply.dataset.commentAuthor||'网友';const floor=tianyaFloorReply.dataset.floor||'';const content=windowRef.prompt?.(`回复 @${who} #${floor}`,'')??null;if(content&&String(content).trim())addPublicWebComment(getScopeKey?.(),tianyaFloorReply.dataset.postId,{author:{type:'user',id:'user',name:getTavernUserContext()?.name||'User'},content,replyToCommentId:tianyaFloorReply.dataset.commentId});renderPublicWeb();return;}
    ...
    const replyRefresh=event.target?.closest?.('[data-action="tianya-replies-refresh"]'); if(replyRefresh){if(replyRefresh.disabled)return;replyRefresh.disabled=true;const old=replyRefresh.textContent;replyRefresh.textContent='[刷新中…]';try{const post=getPublicWebPost(getScopeKey?.(),replyRefresh.dataset.postId);const replies=await generateTianyaReplyRefresh({scopeKey:getScopeKey?.(),post});for(const reply of replies)addPublicWebComment(getScopeKey?.(),post.id,reply);renderPublicWeb();}catch(error){console.error('[moli小手机] tianya replies refresh failed:',error);windowRef.alert?.(`刷新回复失败：${error?.message||error}`);}finally{replyRefresh.disabled=false;replyRefresh.textContent=old;}return;}
    const comment=event.target?.closest?.('[data-action="public-web-comment"]'); if(comment){const content=windowRef.prompt?.('回复内容','')??null;if(content&&String(content).trim())addPublicWebComment(getScopeKey?.(),comment.dataset.postId,{author:{type:'user',id:'user',name:getTavernUserContext()?.name||'User'},content});renderPublicWeb();return;}
  });
  renderPublicWeb();
  const syncGhostStoriesToggle=()=>{const button=panel.querySelector('[data-action="toggle-ghost-stories"]');if(!button)return;const enabled=getPublicWebSettings(getScopeKey?.()).ghostStoriesEnabled;button.setAttribute('aria-pressed',String(enabled));button.textContent=`莲蓬鬼话：[${enabled?'开启':'关闭'}]`;};
  panel.querySelector('[data-action="toggle-ghost-stories"]')?.addEventListener('click',()=>{const current=getPublicWebSettings(getScopeKey?.()).ghostStoriesEnabled;updatePublicWebSettings(getScopeKey?.(),{ghostStoriesEnabled:!current});syncGhostStoriesToggle();renderPublicWeb();}); syncGhostStoriesToggle();
