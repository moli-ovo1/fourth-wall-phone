...
export function createFloatingBall({ documentRef = document, windowRef = window, uiState, onClick }) {
    const old = documentRef.getElementById('moli-phone-handle');
    old?.remove();
    ...
    handle.addEventListener('pointermove', event => {
        ...
        uiState.handleX = Math.max(0, Math.min(windowRef.innerWidth - HANDLE_SIZE, event.clientX - grabX));
        uiState.handleY = Math.max(45, Math.min(windowRef.innerHeight - HANDLE_SIZE - 15, event.clientY - grabY));
