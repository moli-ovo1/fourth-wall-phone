...
export function recordLifeLog(scopeKey,{actorId='',actorName='',kind='activity',title='',summary='',source='',status='success',metadata={}}={}){const s=load();const row={id:`life:${Date.now()}:${Math.random().toString(36).slice(2,8)}`,scopeKey:String(scopeKey||''),actorId:String(actorId||''),actorName:String(actorName||''),kind:String(kind||'activity'),title:clean(title).slice(0,160),summary:clean(summary).slice(0,2200),source:clean(source).slice(0,120),status:String(status||'success'),metadata:{...metadata},createdAt:Date.now()};s.items.push(row);save(s);return row;}
