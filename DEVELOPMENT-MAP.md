## moli274 / v0.6.27 — 编辑室透明层与气泡动画修正
- 明确确认入口 task 回归已修：用户选择 `帮我想梗 / Ta出场太少啦 / 剧情好难走啊` 后，选择结果保存在 `pendingStudioTask`；用户真正发送文字时，将 task + 用户文字一起传入 `requestReply({ studioTask: task })`。`取纳` 文字也直接进入 adopt task。
- 顶部 nav 改为仅在编辑室浮层显示，不再占据聊天主体 52px 高度；底部 compose 同样浮起。聊天主体本身仍保持原 flex 滚动布局，只增加首尾滚动留白，因此避免 moli272 白屏方案。
- 返回键去掉额外玻璃框；编辑室右上角 `…` 完全隐藏。
- 三个一级入口改用 top/left/right 位移动画，不再依赖容易被历史 CSS 覆盖的 transform，因此 `帮我想梗 / Ta出场太少啦 / 剧情好难走啊` 都持续轻微晃动。
- `取纳` 与注入后的 `清空本次讨论 / 我还没完呢` 浮在输入框上方，不再挤压聊天主体。
- 不改编辑室主 Prompt、moli268 上下文隔离和 moli262 跨墙生命周期。

## moli273 / v0.6.26 — 编辑室白屏回滚重做
- 本版从最后已确认能正常显示编辑室的 moli271 重做，不继承 moli272 对 `.moli-chat-body`、页面和输入区的绝对定位修改。
- 去透明蒙版改为只处理 nav/footer 自身背景、backdrop 与伪元素，不改变聊天主体布局尺寸/定位。
- 编辑室隐藏右上角 `…`；返回键单独保留小玻璃底。
- 三个一级入口恢复轻微晃动；新用户首次进入显示晃动箭头指向 `来嗑瓜子`，点击后不再重复。
- 成功注入素材栏后显示 `清空本次讨论 / 我还没完呢`；清空只清当前编辑室消息，不删除已投入素材栏/我们的墙的内容。
- 同时保留入口 task 在用户发送后真正进入生成请求、以及 `取纳` 文字触发。
- 不改编辑室主 Prompt、moli268 上下文隔离和 moli262 跨墙生命周期。
