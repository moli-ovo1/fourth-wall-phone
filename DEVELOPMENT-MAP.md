## moli272 / v0.6.25 — 编辑室无蒙版、首次引导与讨论生命周期
- 编辑室顶部/底部不再使用整条透明蒙版：聊天滚动层铺满页面，顶部只保留返回键与 `来嗑瓜子`，底部只保留输入框和必要的小气泡，因此消息不会再被透明横条截断。
- 编辑室右上角 `…` 隐藏；返回键拥有自己的小玻璃点击底板。
- 新用户首次进入编辑室时显示一个晃动箭头指向 `来嗑瓜子`；点过一次后本地记忆，不再重复教学。
- `帮我想梗 / Ta出场太少啦 / 剧情好难走啊` 三个一级入口均恢复轻微晃动。
- 每次成功把编辑室内容注入素材栏后，弹出 `清空本次讨论 / 我还没完呢` 两个玻璃气泡。清空只删除当前编辑室聊天记录，不删除已投入素材栏/我们的墙的内容；继续则保留上下文。
- 修复 moli269-271 入口任务在“用户输入并发送”后未真正携带所选 task 的回归；`取纳` 文字发送也恢复直接触发取纳生成。
- 不改 moli268 上下文隔离、编辑室主 Prompt 和 moli262 跨墙生命周期。
