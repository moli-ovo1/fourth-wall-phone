...
## moli253 / v0.6.06 — 娘家人独立创作协作合同
- 「娘家人」虽然复用群聊存储与 JSON 传输层，但产品语义不是微信普通群聊/围读会；不得继承普通群聊气泡配额、全员轮流或微信聊天行为预设。一次请求以当前创作任务完成为停止条件。
- 非固定任务允许小上帝/moli 按需要连续接话、补充、反驳或提前结束；固定三版任务（纳取/规划候选）仍严格输出三版。解析层的高上限只做异常保护，不作为目标气泡数。
- 「帮我想梗」显示名改为「先磕点瓜子再说」，原 meme 内部 task id 保留以避免数据迁移。
- 娘家人聊天页返回键回「我们的墙」，不进入微信聊天列表。
- 娘家人跨墙内容写成开放“事件种子”：只保留外部条件、事件机会、人物本人已有依据的行动入口；讨论阶段的心理猜测不进入候选。正文 Prompt 不暴露 User 的点击/纳取/跨墙 provenance。
...
- **固定人格**：moli / 小上帝 / 皮下仍是单入口；正文内优先当前正文实例，正文外优先 Global。三人格是否进入社区自主生成候选由 User 的「三人格参与社区」开关决定；关闭时仍可通过 User 主动 @ / 邀请 / 转发 / 自创场景召入。该开关只控制社区自主参与资格，不改变三人格“非正文原生人物”的数据归属。
...
- moli / 皮下 / 小上帝仍是跨世界固定人格，不属于正文社区原生人物池；社区自主参与由 User 的「三人格参与社区」开关控制。关闭时不作为自主生成候选，但 User 主动 @、邀请、转发或自创内容明确召入仍可参与；开启时可自然参与，但不要求每轮出现。
...
## Community Social Network v1：微博整站刷新与网络人物连续性
### 微博刷新预算
- 一次点击刷新 = 一次微博生成 API 调用。
- 同一次返回三个内容池：首页 3~5 条、超话 3~5 条、热搜 3~5 条；总计 9~15 条。
- 逻辑分区不要求 API 分调用。三个池必须使用明确 lane：`关注/同城/实时`、`超话`、`热门`。
- 每池普通 AI 历史上限 15 条；User 自发内容、置顶内容不作为普通淘汰对象。

### Network Actor 连续性
- 🔥持续网友只记自己真正经历的互动，不因为后台存在整页内容而获得整页知识。
- 微博公开互动与微博私信都可以成为其有界持续记忆来源。
- `+微信好友` 是 User 显式把持续网络人物升级为微信联系人的入口；升级后继承已有公开画像/真实互动经历，并保持 Network Actor ↔ Contact 映射避免重复创建。
- 普通“添加联系人”不提供网友来源选择；网友来源来自 Community 的真实网络人物升级路径。

### Character Batch 成本优化约束
- 邀请/@ 1~3 个 Character 的目标是一次 Actor Batch 调用，路人维持独立批量调用。
- 合并前提：每个 Character 只能读取自己的角色卡、手机经历、认知与当前公开帖子。若底层请求无法保证私有 Actor Pack 隔离，则保持逐角色调用，不能用知识串线换 API 成本。

## Community 初始互动与 Network Actor（v0.5.78）
- “用户体验感 > 模拟多数现实帖子冷清”：所有开启讨论区的 Community 内容首次出现时，按内容价值生成可参与的初始互动。普通约 5–8，活跃约 8–12，热门/争议约 12–18。互动可以由顶层与下级回复共同构成；后续手动刷新评论区仍沿用原追加规则。
- 平台细分规范必须留在各平台 Community Preset：天涯强调楼层关系；小红书强调评论/作者回复/细节支线；知乎以回答+回答下评论共同构成；微博按首页/超话/热搜形成不同回复生态；自创服从 User 自定义结构。
- 微博刷新是一轮整站变化：一次 API 生成 8–10 条，三池都必须有内容，不机械平均；存储仍按三池分别最多 15 条普通 AI 内容。
- User 发微博的大号/小号选择必须复用 Community ID card 已保存身份，不另建第二套 User 小号数据库。
- Network Actor 是持续网络人物，Public ID 是其公开账号身份。稳定公开 ID 的实际公开经历可以进入该 Network Actor 自己的有界连续性；后台归属关系不等于其他人物认知。

### Weibo feed / hot-search contract (v0.5.79)
- 首页+超话是正常刷新生成的微博内容池：一次 8~10 条。
- 热搜榜是独立的 5~8 个 `#关键词#`，不对应可点击帖子入口。
...
### Community 自主浏览认知闭环（本轮补充）
- 公共 Community（天涯 / 小红书 / 知乎 / 微博 / 推荐）在一次刷新生成并落地帖子后，追加一次**批量自主浏览判断**；它与朋友圈的 `viewedMomentIds` 属于同一设计思想。
- 当前正文世界人物无需 User `@`、邀请或转发，也可能自行逛社区并看到本轮帖子。浏览结果只记录本轮人物实际看到的 `postId`；允许 0 人浏览、允许某人物 0 帖已阅，不得把后台提供的全部帖子自动变成人物知识。
- “看见帖子”与“公开行动”严格分离：谨慎、身份敏感、不适合实名发言等，只能影响人物是否公开行动，不能被推导成“因此不会浏览”。
- 实际看见的帖子继续复用现有 `POST_SNAPSHOT_KNOWN -> World Event / Awareness -> buildPhoneContext()` 链进入人物统一手机认知；不新建 Community 专属平行记忆。
- 三人格是否进入这次自主浏览候选，服从「三人格参与」开关；关闭时仍保留 User 主动召入路径。
- 微博不再以“与当前公开事件自然相关”作为 Character 本人账号/已有小号的参与资格门槛。已有身份可持续使用，是否行动由人物状态、动机与当轮情境决定；仍不要求每轮出现。


## v0.5.84 / moli231 — Community 自主生态闭环
- 公共 Community 的人物行为拆成四条独立决策轴：自主浏览、看到后的公开参与、独立主动发帖、Community → 微信主动联系。不得再用单一“参与率”把四者绑定。
- 自主浏览结算可产生：只看不说、实名回复、人物自己的小号/匿名回复、微信 MESSAGE、真实帖子 SHARE、或 SKIP；公开参与与私聊彼此独立。
- 私聊新增人物级权限「允许社区触发主动私聊」。只有总「主动私聊」权限和该开关同时开启，Community 才能成为微信主动联系来源；关闭不影响人物浏览、评论或发帖。
- SHARE 使用真实 Community Forward 消息卡片写入微信；MESSAGE 只因社区经历产生自然私聊。Prompt 不预定义角色话术。
- 人物主动发帖是独立动机，可与浏览/评论频率完全不同；一次刷新允许无人浏览、无人评论、无人发帖、无人私聊。
- User 本人账号与 User 创建/绑定的社区大号、小号属于 User-controlled identity：AI 自主生成与后续刷新不得代替 User 使用这些身份；已由 User 真实发布的公开内容仍可被人物自主看见并反应。
- 评论关系以 replyToCommentId 为真实父节点。首次批量生成若模型用“回复@某网友”表达，会解析到同批已存在的目标评论并去除展示文字前缀；后续刷新同样优先真实 parent id，避免伪装成回复的新一级楼。天涯仍保持线性楼层，但 replyToCommentId 保存真实引用关系。
- 微博 5~8 条热搜扩展为整个当前社会的窗口：新闻、娱乐、民生、公共事件、互联网争议、网络热点、行业/地方事件均可自行发生；正文人物相关热搜只是可能的一部分，不固定占比，不强制关联 Character。
- 所有真实浏览/公开行动/私聊继续复用 World Event / Awareness / Phone Context，不新增 Community 平行记忆。
...
## moli238 / v0.5.91 — Private Phone Trace 旧数据迁移修复
- 修复 moli234 已存在人物记录升级到 moli235+ 后刷新时报 `Cannot read properties of undefined (reading 'push')`：旧 row 只有 searches/views 时，写入新 MEMO 会访问不存在的 memos 数组。
- `private-phone-trace-store` 现在在 Character row 入口统一做 schema normalization：保留已有 searches/views，缺失或非法的 memos/searches/views 自动归一为空数组，sexualTrace/default metadata 同步补齐。
- 不清空旧「他的手机」记录，不改 Community / WeChat / Automation / Network Actor；本包仅修 Private Phone Trace 持久化兼容。
...
- 「我们的墙」从“素材可进入正文”进一步收口到“正文 AI 能按故事世界语义正确理解素材”：最终 Tavern 一次性注入不再依赖「我们的墙」等产品内部术语解释知识边界，而明确区分既有手机世界事实、人物知识边界、公开账号身份边界与 User 明确手写的创作指导。
- 一次性正文注入明确声明：材料是连续性依据，不是本轮必须逐项完成的任务清单；只在当前人物、场景与叙事自然需要时体现影响，避免为了消费素材机械逐条演出。
- 「他的手机 · 备忘录」Semantic Payload 改为世界内可理解表达：默认只有备忘录主人知道；只有实际查看手机、被本人告知或通过其他合理剧情途径获知的人物才能取得该知识。删除“投入我们的墙”等正文 AI 不应理解的内部产品措辞。
- Community 跨墙头改为“公开互联网”语义：公开存在 ≠ 所有人已经看过；匿名/小号展示身份 ≠ 真实主人。后台能够关联身份不得直接转化为正文人物识破身份。
- 微信公共/角色朋友圈与手机关系记忆补齐同类知识边界；关系记忆继承原私聊/群聊参与者边界，不把 memory summary 当作全世界共享事实。
- 保留 moli240 的开放 Wall Source Provider、结构化 Community ★ 选择、现有 Workspace/History、成功消费/停止保留的一次性 Tavern Injection 生命周期；本版不改选择 UI，不增加自动跨墙。
...
## moli244 / v0.5.97 — 「娘家人」入口与交互收口
- 「编剧室」前台正式改名为「娘家人」；内部 `writers-room` systemKind / storage key 保持不变，避免无意义迁移。旧 v0.5.96 房间进入时自动把显示名迁移为「娘家人」。
- 「娘家人」只从「我们的墙」右上角进入；该 system room 从微信聊天列表过滤，不再与「围读会」并列显示。底层仍复用既有群聊消息/生成能力，不复制第二套聊天引擎。
- 使用 User 提供的 `assets/writers-room/niangjiaren.jpg` 作为「娘家人」默认专属壁纸；若 User 后续为当前会话手动设置专属壁纸，手动设置优先。全局微信壁纸不覆盖娘家人默认壁纸。
- 顶部胶囊工具栏移除整条白色底板，仅保留胶囊；删除「整理❤️」按钮。❤️继续沿用 v0.5.96 语义：只本地标记，点亮本身 0 API；下一次正常发送/任务生成时与新输入一起作为参考信号提供给小上帝与 moli。❤️不是命令、不是永久偏好，也不要求二人格逐条回应。
- 本版刻意不实现「剧情规划长线」与「生活灵感长线」。两者仍处于设计打磨阶段：前者拟持续观察正文并允许编辑/暂停/取消/转向；后者拟在符合当前时间地点人物环境时提供非工具化的生活扰动与小事件。不得在语义/触发频率/API成本机制未确认前提前落地。


## moli245 / v0.5.98 — 娘家人「帮我想梗」三岔入口 + 生活灵感观察 v1
- 修复 moli244 工具栏作用域回归：娘家人四胶囊只允许在 `type=group && systemKind=writers-room` 的专属会话显示；微信私聊、正文聊天、皮下、围读会等均不得出现。
- 工具栏改为正常页面流中的独立胶囊行，透明背景、无 blur/蒙版；胶囊自身提高不透明度，消息正文不得钻入工具栏遮罩后被截断。
- 「帮我想梗」点击后不再直接生成，先出现「你先说 / 我先说 / 都别说了系统来办」。你先说=娘家人直接读正文发散；我先说=关闭选择层并聚焦输入框，等待 User 先给点子，下一次正常发送继续携带既有❤️参考。
- 主动“帮我想梗”共享生活扰动原则：从时间、地点、季节、环境、身份、社会关系、生活习惯、工作状态和既有世界事实寻找小事/偶遇/麻烦/便利/插曲/小幸运等；生活允许只是发生，不要求服务主线。小上帝偏自然、现实、可落地；moli 允许更大胆、更意外、更有新意，但不能破坏世界事实与人物边界。
- 「都别说了系统来办」二次确认后开启生活灵感观察 v1。它不额外调用娘家人 API，而在正文生成链每累计5次有效生成提供一次低频导演机会；正文 AI 可以完全不采用。提示禁止强行制造大人物/阴谋/事故/危机、禁止无关配角硬闯、禁止常见套路短期重复。
- 娘家人页增加生活灵感观察状态条，可暂停/继续/关闭。固定5次生成仅是低成本 v1 的机会节流，不代表最终“静滞判定”算法；后续可在不改变User入口的前提下升级为剧情静滞驱动。
- 剧情规划长线仍未施工；不要与本版生活灵感观察混为一套状态机。
...
## moli249 / v0.6.02 — 跨墙潜伏线注入与状态栏修复（2026-09-21）
- 修复正文「我们的墙」状态栏不出现：`story-bridge-status` 渲染时遗漏读取 `plans`，导致运行期 `ReferenceError` 在 DOM 插入前中断；现在先读取当前 scope 的 active plans，再统一渲染跨墙剧情线 / 生活灵感 / 剧情规划。
- 持续剧情线提示词从“后台状态机说明”为主改成低压力的「跨墙潜伏线」：明确它是长期保留的事件机会，不是本轮任务、不是既成事实；条件合适才自然进入，条件不合适继续潜伏。激活回执保留，但压缩为末尾内部状态说明。
- 持续剧情线改为 `SYSTEM + IN_CHAT + depth 4`；一次性手机上下文仍保持靠近当前轮的 depth 0。该调整用于让长期潜伏线更像背景连续性而不是末端强命令，不代表复制外部插件文本或状态结构。
- 娘家人「纳取」再加源内容净化：讨论原文里的角色心理/欲望/情绪/判断/反应断言必须在三版提示词中删除，不得换句话保留，也不得把“掌控欲/嫉妒/冲动处于某状态”变成事件触发前提。
...
## 全局预设：所有小手机 AI 请求的最高层用户配置（v0.6.08）
- 设置中的 `global` 预设不是微信预设，也不是某个 App 的专属规则；它是整个 moli 小手机的共享用户配置盒子。
- 所有需要调用 AI 的现有与未来 App / 后台生成任务，默认都必须继承该全局预设：微信私聊/群聊/围读会、先磕点瓜子再说、朋友圈、Community/天涯/小红书/知乎/微博、他的手机，以及后续新增 App。
- Prompt 顺序固定为：**全局预设 → 当前 App/功能系统规则 → 人物/世界/记忆/正文等上下文 → 本轮任务与用户输入**。
- App 专属预设（如微信线上聊天预设、Community 预设）不得重复携带 global 内容；global 由统一生成入口单独置顶。
- 新 AI 功能应走统一生成入口，避免开发者忘记手工接入 global；只有完全不调用 AI 的功能不存在该注入。
...
## moli257 — 角色设定语义与编辑室 NPC 事实链
- “角色设定”是语义概念；SillyTavern 角色卡与世界书是存储来源，不能固定解释为“人设 vs 世界背景”。世界书中的人物身份、关系、经历同样属于角色设定。
- 编辑室识别/讨论 NPC 时，不使用可由 User 单独编辑的微信联系人资料定义 NPC。事实检索顺序：当前 char 角色描述与相关世界书 → 柏宝书长期记忆 → 当前正文上下文。前两者确认人物既定设定，柏宝书确认既往历史，正文确认当前阶段。
...
- 编辑室取纳必须读取真实 `likedByUser` 爱心气泡作为User兴趣信号；输入框上方固定提供“取纳”入口，取纳成功后同时出现素材栏跳转提示。编辑室爱心按钮保持无外框透明样式。

- writers-room Prompt 保持最小化：普通聊天不设置“自由讨论任务/一次请求完成任务”等额外模式；只保留编辑室人格、后台资料、聊天历史与❤️信号。只有显式按钮任务或“取纳”才临时加入对应任务指令。废弃 Prompt 必须从最终拼装链删除而非保留兼容文案。

- 编辑室总人格以 ①–⑦ 简洁规则为唯一聊天行为层：故事资料是前情提要而非谈资；已有讨论后，后续轮次必须承接用户最新发言，不因重新读取正文而自动重置讨论话题。


## 编辑室上下文边界（moli268）
- `writers-room` 是独立编辑室模式，不是围读会的换皮实现。
- 编辑室可以后台读取当前故事事实作为前情提要，但不得接收围读会的“共享当前正文辅助上下文”块或点评任务语义。
