## moli274 / v0.6.27 — 编辑室 UI 收口
- 确认并保留：入口选择后，用户发送文字时会把 `pendingStudioTask` 真正传入 `requestReply({ studioTask })`；`取纳` 文字也会触发 adopt。
- 返回键恢复透明干净样式，不再加单独玻璃方框。
- 顶部蒙版保留；右上角 `…` 恢复。编辑室点击 `…` 时向左展开 `清空聊天记录`，再次点 `…` 收起；普通聊天的 `…` 行为不变。
- `Ta出场太少啦 / 帮我想梗` 的晃动改用 margin-top 动画，绕开旧 `transform!important` 锁死动画的问题。
- 底部不改聊天主体布局，只取消编辑室工具容器自身的额外背景/模糊；玻璃遮罩只由真正的输入框胶囊承担。
- 保留首次箭头、取纳后的 `清空本次讨论 / 我还没完呢`、moli268 上下文隔离与 moli262 跨墙生命周期。
