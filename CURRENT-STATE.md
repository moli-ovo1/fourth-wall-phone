...
## v0.5.40 stable rollback correction
- Clarification: the observed regression was not that a forwarded Community post could never be resolved. The regression symptom was conversational reference continuity: after a role already had the Community post context, a follow-up such as “那个匿名的是我” should let the role connect that disclosure to the known post instead of claiming it only has fragments or must guess.
- The v0.5.38 Knowledge Delta protocol is frozen/removed from the active generation baseline. Do not inject identity-candidate IDs or machine `<knowledge_delta>` instructions into normal private/group roleplay while stabilizing.
- Community runtime remains on the pre-unified-settlement stable generation path (v0.5.34 generation + pre-v0.5.35 Community UI orchestration). This is intentional to restore ordinary netizen growth and separate character invite/@ generation.
- Community -> Conversation forward references remain supported by the pre-existing `communityForward` + `resolvedContext` object-resolution path. Do not replace this with copied summary text.
- Reverse Conversation -> Community knowledge propagation remains deferred until the forward/reference and Community ecology regressions are verified stable.
