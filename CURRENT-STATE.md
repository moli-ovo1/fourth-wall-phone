...
## moli142 / v0.4.82 — World/Social Event Pipeline 第一完整闭环
- User 朋友圈“谁可以看”废弃旧 sheet/page 入口，改为发表页原地展开纯文字角色名单；不显示头像。未选择任何角色=公开，选择后=仅所选角色可见。
- 名单按角色原名展示；酒馆正文/全局联系人附 `（正文）` / `（全局）`，自建/内置角色直接显示名字。
- 可见范围不仅保存 UI 值：公共朋友圈批量刷新时，限定可见动态不会进入所有角色共享上下文，只进入被允许角色自己的可见区，避免角色越权看到。
- World Event Store 增加 known/pending、消费标记、短时去重、聚合摘要和统一 Context Source 出口。事实、认知、行动继续分离。
- 朋友圈已结算事件进入角色后续手机连续性；SKIP 不等于遗忘。
- 社区 @ 与知乎邀请开始登记统一 world event，并在明确点名时直接作为角色已知事实；是否公开回应/私聊仍由角色自行决定。
- 主动私聊默认开启继续沿用；主动程度仍是 0–100 人物倾向，不是随机骰子。
- 下一大节点：Phone Context Injection，基于 `phone.world-events` Context Source 做手机→正文的可选择穿墙，不把注入写死为微信专用。

## v0.4.83 / moli143 — 自创社区 + 可见范围降维 + 世界事件入墙（2026-09-16）
- 施工前重新审计了社区真实链路：`public-web-store.js`（统一公共网络数据仓）→ `generation-service.js`（天涯/小红书/知乎/推荐生成与评论生长）→ `phone-panel.js`（推荐、详情、评论、@、知乎邀请、微信桥）→ `world-event-store.js`。自创社区建立在这条现有链上，不另造孤立 App/评论系统。
- moli社区顶部新增“自创”。User 可创建多个自创社区，填写“名称 + 想看什么/世界范围/参与者/气质”，并启用、停用、编辑、删除。启用项会作为候选生态进入“社区推荐”同一次刷新；AI 在固定社区与启用自创社区之间自然挑选 4~6 条，不按平台机械配额。
- 自创推荐内容永久归档到 `section=custom`，携带 `customCommunityId/customCommunityName/customCommunityDescription`；“自创”页可查看已生成内容，详情沿用公共网络评论/@链，并有独立的评论继续生长 Prompt，不强行套天涯/小红书/知乎语法。
- 自创社区的生成、User 评论、@角色开始进入 `community.custom` World Event。@仍是明确点名：人物获得判断机会，但可 REPLY / MESSAGE / BOTH / SKIP。
- User朋友圈“谁可以看”不再尝试弹层/跳页/点击展开：发表页常驻纯文字角色名单；不选=公开，选择后=仅所选联系人可见。现阶段可见性仍以 contactId 为知识边界；不要伪造“正文/全局 Conversation 已能分别授权”的能力，后续若要把同一联系人不同 Conversation 当作两个可见性主体，需要先升级 visibility 数据模型和公共朋友圈候选主体。
- 修复公共朋友圈批量刷新里一个旧的 `contact.id` 未定义过滤路径：公共 feed 与“仅某联系人可见”的私有 feed 重新分开组装。
- “我们的墙”没有重做，也没有长期绑定。已有单轮手选注入继续保持；本版只把 `phone.world-events` 已知事件作为新的可选素材类别接入现有素材树，由 User 自己勾选、编辑、决定是否跨墙。
- 重要产品铁律：我们的墙不做自动长期绑定；旧 TODO 不能证明功能未完成，开始大功能前必须以 CURRENT-STATE + 实际代码调用链核验。
